#!/usr/bin/env python3
"""Assemble the exact S72 accepted-support certificate."""
...
def main():
    moments = json.loads(Path("moments_d128_s72_a38_b5.json").read_text())
    ...
    certificate = {
        "description": (
            "Exact accepted-support certificate for the D128 S72 mixed shell: "
            "38 signed coefficients of magnitude one and five signed coefficients "
            "of magnitude two on positions with 2-adic valuation zero or three."
        ),
        "method": "degree-30 exact moment dual, exact polynomial positivity, union bound",
        "params": {
            "d": DIMENSION,
            "selected_positions": SELECTED_POSITIONS,
            "position_predicate": "1 <= i < 128 and v2(i) in {0,3}",
            "a_mag1": MAG1_COUNT,
            "b_mag2": MAG2_COUNT,
            "w_nonzero": WEIGHT,
            "l1_norm": L1_NORM,
        ...
        "moment_generation": {
            "algorithm": (
                "exact truncated mixed-shell bivariate EGF over the 72 selected "
            ...
            "a_priori_numerator_bound": "C(72,43)*C(43,5)*2304^m for moment m",
        },
    }
    Path("cert_d128_s72_a38_b5_gamma18.json").write_text(
