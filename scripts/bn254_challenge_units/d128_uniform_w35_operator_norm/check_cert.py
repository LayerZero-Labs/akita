...
def main():
    ...
    cert_path = (
        Path(arguments[0])
        if arguments
        else Path(__file__).with_name("cert_d128_uniform_w35_gamma14.json")
    )
    ...
    check("uniform candidate dimensions", d == 128 and w == 35 and b == 0)
    check("B=l1^2", B == params["l1_norm"] ** 2 == 1225)
    check("nominal dual threshold is 14", nominal_gamma == 14)
    check("64 conjugacy-distinct odd frequencies", nfreq == d // 2 == 64)
    check("raw support", str(nraw) == cert["N_raw"])
    check("M0=1 and M1=35", moments[0] == 1 and moments[1] == 35)
    ...
    check("accepted support is at least 2^136", F(nraw) * p0 >= 2**security_bits)
