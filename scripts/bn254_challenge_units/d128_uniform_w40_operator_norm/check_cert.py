...
def main():
    ...
    cert_path = (
        Path(arguments[0])
        if arguments
        else Path(__file__).with_name("cert_d128_uniform_w40_gamma15.json")
    )
    ...
    check("uniform candidate dimensions", d == 128 and w == 40 and b == 0)
    check("B=l1^2", B == params["l1_norm"] ** 2 == 1600)
    check("nominal dual threshold is 15", nominal_gamma == 15)
    check("64 conjugacy-distinct odd frequencies", nfreq == d // 2 == 64)
    check("raw support", str(nraw) == cert["N_raw"])
    check("M0=1 and M1=40", moments[0] == 1 and moments[1] == 40)
    ...
    for prime in cert["moment_generation"]["reconstruction_primes"] + [cert["moment_generation"]["independent_check_prime"]]:
        ...
        for m in range(degree + 1):
            ...
            if numerator_sum.denominator != 1 or numerator_sum.numerator % prime != residues[m]:
                residue_checks = False
                break
    check("all moments match nine modular generator outputs", residue_checks)
    ...
    check("accepted support meets certificate floor", F(nraw) * p0 >= 2**security_bits)
