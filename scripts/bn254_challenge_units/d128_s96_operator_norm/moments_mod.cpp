...
int main(int argc, char **argv) {
    ...
    auto convolve_add = [&](int source, int destination,
        ...
        for (int index = 0; index < static_cast<int>(area); ++index)
    ...
    for (const int position : positions) {
        ...
        ++processed;
        const int max_a = std::min(mag1_count, processed);
        for (int a = max_a; a >= 0; --a) {
            const int max_b = std::min(mag2_count, processed - a);
            for (int b = max_b; b >= 0; --b) {
                if (a + b == 0 || a + b > processed) continue;
    ...
    std::cerr << "selected_positions " << positions.size()
