#!/usr/bin/env python3
"""Verify the exact D128 S96 accepted-support certificate with stdlib only."""
...
def main():
    ...
    certificate_path = (
        Path(arguments[0])
        if arguments
        else Path(__file__).with_name("cert_d128_s96_a35_b1_gamma15.json")
    )
    ...
    positions = [index for index in range(1, dimension) if index % 4 != 0]
    check(
        "S96 structural parameters",
        dimension == 128
        and selected_count == len(positions) == 96
        and mag1_count == 35
        and mag2_count == 1
        and weight == 36,
    )
    ...
    check("B=l1^2", support_cap == params["l1_norm"] ** 2 == 1369)
    check("challenge energy is 39", params["l2_sq"] == 39)
    check("nominal threshold is 15", nominal_gamma == 15)
    ...
    check(
        ...
        moments[0] == 1 and moments[1] == 39 and moments[2] == expected_second,
        f"M2={moments[2]}",
    )
