...
Collision bucket convention:
    Each bucket is the whole-vector squared Euclidean collision bound
    `collision_l2_sq` (an exact integer). The scalar SIS Euclidean length bound
    passed to lattice-estimator is `sqrt(collision_l2_sq)`.
...
Modeling choices (matching the existing table):
    ...
    - length_bound = sqrt(collision_l2_sq)  (whole-vector squared key)
...
def print_provenance_header(
    ...
    print(f"// Model: ADPS16 quantum, Euclidean (norm=2)")
