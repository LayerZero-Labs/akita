...
Options:
    ...
    --search-cap N          Upper bound for binary search (default: 10^10).
...
The existing table was generated with a search cap of 5M (D=32), 20M (D=64),
or 50B (D=128). This script defaults to 10^10 for all dimensions.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--search-cap", type=int, default=10_000_000_000,
        help="Upper bound for binary search (default: 10^10)."
    )
    parser.add_argument(
...
def main() -> None:
    ...
    print(f"// Model: BDGL16 + lgsa, q = 2^128 - 275")
    print(f"// Target: {args.target_bits} bits, search cap: {args.search_cap:_}")
    ...
    for d, collision in entries:
        ...
        for rank in range(1, MAX_RANK + 1):
            ...
            w = binary_search_max_width(
                ...
                args.target_bits, args.search_cap,
            )
            elapsed = time.time() - t0
            print(
                f"        // (d={d}, collision={collision}, rank={rank}): "
                f"max_width={w:_} ({elapsed:.1f}s)",
                file=sys.stderr,
            )
            widths.append(w)
