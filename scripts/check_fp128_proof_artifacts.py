#!/usr/bin/env python3
"""Check that the proved and production Fp128 subtraction words agree."""
...
OBJECT_WORDS = [*SUB_BODY, RET]
PRODUCTION_WITNESS_WORDS = [LOAD_A7F7_INTO_W4, *SUB_BODY, RET]
...
def main() -> None:
    ...
    object_words = read_symbol_words(tool, args.sub_object, "akita_fp128_sub_asm")
    witness_words = read_symbol_words(
        tool,
        args.production_witness,
        "akita_fp128_sub_production_witness",
    )
    require_words("standalone proof object", object_words, OBJECT_WORDS)
    require_words(
        "production subtraction witness",
        witness_words,
        PRODUCTION_WITNESS_WORDS,
    )
    print("Fp128 subtraction proof object and production witness bytes match.")
