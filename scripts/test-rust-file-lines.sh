...
write_generated() {
    ...
    mkdir -p "$(dirname -- "$file")"
    {
        printf '// Generated by `some-emitter`\n'
        awk -v lines="$((lines - 1))" 'BEGIN { for (i = 1; i <= lines; i++) print "// line " i }'
    } > "$file"
}
...
repo="$(new_repo)"
write_generated "$repo/src/generated.rs" 9
write_baseline "$repo"
track_all "$repo"
expect_success "generated file over cap is exempt" "$repo"

repo="$(new_repo)"
write_generated "$repo/src/generated.rs" 9
write_baseline "$repo" $'9\tsrc/generated.rs'
track_all "$repo"
expect_failure "generated file must not be baselined" "$repo" "machine-generated (cap-exempt); remove its baseline entry"

repo="$(new_repo)"
# A normal file that merely mentions "Generated by" below the first line is
# still checked: only the first physical line acts as the marker.
write_lines "$repo/src/not generated.rs" 6
sed -i.bak '2s|.*|// Generated by something|' "$repo/src/not generated.rs" && rm -f "$repo/src/not generated.rs.bak"
write_baseline "$repo"
track_all "$repo"
expect_failure "marker only counts on first line" "$repo" "above the cap of 5"
