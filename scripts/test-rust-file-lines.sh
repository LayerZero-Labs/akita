...
write_generated_lines() {
    ...
    mkdir -p "$(dirname -- "$file")"
    {
        ...
        awk -v lines="$lines" 'BEGIN { for (i = 2; i <= lines; i++) print "// generated line " i }'
    } > "$file"
}
...
repo="$(new_repo)"
write_generated_lines "$repo/src/generated_table.rs" 6
write_baseline "$repo"
track_all "$repo"
expect_success "generated Rust file is ignored by line cap" "$repo"
