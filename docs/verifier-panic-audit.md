...
| In-memory terminal responses cannot panic on malformed metadata/buffers. | `TerminalResponse` decode paths return `Result`; malformed-shape tests live in `crates/akita-types/src/proof/tail_segments/tests.rs`, and terminal replay validates the scheduled layout in `crates/akita-verifier/src/protocol/core/terminal_direct.rs`. | Guarded |
