...
- **`test-all-schedules-drift`** runs the exact generated-table-vs-planner comparison outside the timing artifact (`cargo test -p akita-config --features all-schedules …`). Other jobs consume the committed generated catalogs instead of redundantly regenerating them.
