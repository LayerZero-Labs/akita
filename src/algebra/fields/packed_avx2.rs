    ...
    fn add(self, rhs: Self) -> Self {
        unsafe {
            ...
            let result = if Self::BITS <= 31 {
                ...
                let t = _mm256_add_epi32(a, b);
                // Emulate unsigned compare: XOR with 0x80000000 converts u32 compare to i32
    ...
    fn sub(self, rhs: Self) -> Self {
        unsafe {
            ...
            let result = if Self::BITS <= 31 {
                ...
                let corrected = _mm256_add_epi32(t, p);
                _mm256_blendv_epi8(t, corrected, underflow)
            };
...
// ===== PackedFp128Avx2 (2-wide) =====

/// Number of `Fp128` lanes in an AVX2 packed vector.
pub const FP128_WIDTH: usize = 2;

/// AVX2 packed arithmetic for `Fp128<P>`, processing 2 lanes (scalar delegation).
#[derive(Clone, Copy)]
#[repr(transparent)]
pub struct PackedFp128Avx2<const P: u128>(pub [Fp128<P>; FP128_WIDTH]);
...
impl<const P: u128> fmt::Debug for PackedFp128Avx2<P> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_tuple("PackedFp128Avx2").field(&self.0).finish()
    }
}
