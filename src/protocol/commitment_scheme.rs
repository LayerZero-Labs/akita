...
pub(crate) struct SetupCarry<F: FieldCore> {
    ...
    /// on the prover side and the matching verifier cache on the verifier
    ...
    ScheduleCfg: crate::protocol::shared_matrix_setup::SharedMatrixOpeningConfig<Field = F>,
{
    ...
    // `emitted_carry` is `Some` only when the Fused path emitted a fresh
    ...
    let emit_carry =
        level == 0 && delegation_ctx.is_some() && matches!(mode, HachiProtocolMode::Fused);
    let (stage1_proof, stage2_sumcheck, sumcheck_challenges, w_eval) = match mode {
        ...
        HachiProtocolMode::Fused => {
            if emit_carry {
                let sm_setup = delegation_ctx.expect("emit_carry implies delegation_ctx is Some");
                let out = prove_fused_stage1_stage2_emitting_carry::<F, T, D>(
                    transcript,
                    w_evals_compact,
                    tau0_reordered,
                    b,
                ...
                emitted_carry = Some(out.setup_carry);
                (
                    out.stage1_proof,
                    out.stage2_sumcheck,
            ...
            } else if carry_consumable {
                let carry = incoming_setup_carry
                    .as_ref()
                    .expect("carry_consumable implies incoming_setup_carry is Some");
                let out = prove_fused_stage1_stage2_with_carry::<F, T>(
                    transcript,
                    w_evals_compact,
                    tau0_reordered,
                    b,
    ...
    Cfg: crate::protocol::shared_matrix_setup::SharedMatrixOpeningConfig<Field = F>,
{
    if level_d == D {
        ...
        // shared-matrix delegation context (a level-D-typed
        // `SharedMatrixSetup`), so `delegation_ctx` is dropped here. The
        // setup-claim carry, on the other hand, is ring-dimension agnostic
        // (only flat field data) and is threaded through regardless.
    ...
    Cfg: crate::protocol::shared_matrix_setup::SharedMatrixOpeningConfig<Field = F>,
{
    ...
    fn setup_verifier(setup: &Self::ProverSetup) -> Self::VerifierSetup {
        let mut verifier_setup = setup.verifier_setup();
        verifier_setup.shared_matrix_cache =
            crate::protocol::shared_matrix_setup::build_shared_matrix_verifier_cache::<F, D>(setup)
                .ok();
        verifier_setup
    }
    ...
    fn prove<T: Transcript<F>, P: HachiPolyOps<F, D>>(
        ...
        // Build the shared-matrix carry context once at outer D, when
        // setup-claim carry is enabled. Recursive levels never re-materialize
        // it, so the outer-D cache stays consistent with the verifier-side
        // `SharedMatrixVerifierCache`.
        let delegation_setup = if setup.carry_setup_claim {
            Some(crate::protocol::shared_matrix_setup::SharedMatrixSetup::<
                F,
            >::from_main_prover_setup::<D>(setup)?)
        } else {
            None
        };
    ...
    T: Transcript<F>,
{
    ...
    if stage1.fused_leaf.is_some() {
        ...
        let m_eval_at_x_challenges = |r_x: &[F]| -> Result<F, HachiError> {
            prepared_m_eval.eval_at_point::<D>(
                r_x,
                &setup.expanded,
        ...
        return verify_fused_stage1_stage2::<F, T, _>(
            transcript,
            stage1,
            &stage2.sumcheck,
            relation_claim,
...
// ---------------------------------------------------------------------------
// Fused Stage 1 + Stage 2 helpers (HachiProtocolMode::Fused)
// ---------------------------------------------------------------------------

/// Output of [`prove_fused_stage1_stage2`].
struct FusedProveOutput<F: FieldCore> {
    stage1_proof: HachiStage1Proof<F>,
    stage2_sumcheck: SumcheckProof<F>,
    /// Stage 2 (claim-reduction) sumcheck output point `r2`. Used as the
    /// chained opening point for the next recursion level.
    sumcheck_challenges: Vec<F>,
    /// `w(r2)` after Stage 2, used for next-level chaining.
    final_w_eval: F,
}

/// Run the fused (T2) Stage 1 + Stage 2 prover: a non-eq-factored fused leaf
/// (range check + relation), then a degree-3 claim-reduction Stage 2 that
/// fuses s-virtualization and w-adaptation.
///
/// The resulting [`HachiStage1Proof`] has `stages` empty and `fused_leaf`
/// populated; `s_claim` is `w(r1)*(w(r1)+1)` and `w_eval` is `w(r1)`. The
/// caller is responsible for assembling these into a `HachiLevelProof` /
/// `HachiBatchedRootProof`.
#[allow(clippy::too_many_arguments)]
#[inline(never)]
fn prove_fused_stage1_stage2<F, T>(
    transcript: &mut T,
    ...
    relation_claim: F,
    span_level: usize,
) -> Result<FusedProveOutput<F>, HachiError>
where
    F: FieldCore + CanonicalField + FieldSampling + HasUnreducedOps + FromSmallInt,
    T: Transcript<F>,
{
    let gamma_rel: F = transcript.challenge_scalar(CHALLENGE_FUSED_RELATION_BATCH);

    let (stage1_sumcheck, _r_stage1, s_claim, w_eval_stage1) = {
    ...
    let (stage2_sumcheck, sumcheck_challenges, final_w_eval) = {
        ...
        let final_w_eval = {
            let _span = tracing::info_span!("multilinear_eval", level = span_level).entered();
            claim_red.final_w_eval()
        };
...
#[allow(clippy::too_many_arguments)]
#[inline(never)]
fn prove_fused_stage1_stage2_emitting_carry<F, T, const D: usize>(
    transcript: &mut T,
    w_evals_compact: Vec<i8>,
    tau0_reordered: Vec<F>,
    b: usize,
    ...
    sm_setup: &crate::protocol::shared_matrix_setup::SharedMatrixSetup<F>,
) -> Result<FusedEmittingCarryProveOutput<F>, HachiError>
where
    F: FieldCore
        + CanonicalField
        + FieldSampling
    ...
    T: Transcript<F>,
{
    ...
    let m_evals_x_copy = m_evals_x.clone();
    ...
    let setup_evals_x: Vec<F> = m_evals_x_copy
        .iter()
        .zip(&alg_m_evals_x)
        .map(|(m, a)| *m - *a)
        .collect();
...
fn prove_fused_stage1_stage2_with_carry<F, T>(
    ...
    span_level: usize,
    carry: &SetupCarry<F>,
) -> Result<FusedCarryProveOutput<F>, HachiError>
where
    F: FieldCore + CanonicalField + FieldSampling + HasUnreducedOps + HasWide + FromSmallInt,
    T: Transcript<F>,
{
    let gamma_rel: F = transcript.challenge_scalar(CHALLENGE_FUSED_RELATION_BATCH);
...
enum FusedWitnessOracle<'a, F: FieldCore> {
    /// `w(r_cr)` is supplied by a trusted claim absorbed into the transcript.
    ClaimedEval(F),
    /// The final-level witness payload; `w(r_cr)` is computed by directly
    /// evaluating the witness MLE at the stage-2 challenges.
    Direct(&'a DirectWitnessProof<F>),
}

/// Verify a fused Stage 1 + Stage 2 proof.
///
/// Returns the Stage 2 (claim-reduction) sumcheck output point `r2`, which
/// the caller chains as the opening point for the next recursion level.
///
/// `m_eval_at_x_challenges`: closure that evaluates the M oracle at the
/// Stage 2 x-challenges, using the appropriate ring opening point context
/// (single vs batched). `witness`: source for `w(r_cr)`, either a trusted
/// claim (intermediate level) or a direct witness (last level).
#[allow(clippy::too_many_arguments)]
#[inline(never)]
fn verify_fused_stage1_stage2<F, T, MEval>(
    transcript: &mut T,
    ...
    MEval: FnOnce(&[F]) -> Result<F, HachiError>,
{
    ...
    let degree_bound = b / 2 + 1;

    let (final_claim_stage1, r_stage1) = {
        let _sumcheck_span = tracing::info_span!("stage1_sumcheck_fused").entered();
        // Match `prove_sumcheck`'s transcript shape: absorb input claim, then rounds.
    ...
    let (final_claim_stage2, sumcheck_challenges) = {
        let _sumcheck_span = tracing::info_span!("stage2_sumcheck_claim_reduction").entered();
        transcript.append_serde(ABSORB_SUMCHECK_CLAIM, &claim_reduction_claim);
        stage2_sumcheck.verify::<F, _, _>(claim_reduction_claim, num_vars, 3, transcript, |tr| {
            tr.challenge_scalar(CHALLENGE_SUMCHECK_ROUND)
        })?
    };

    let final_w_eval = match witness {
        FusedWitnessOracle::ClaimedEval(v) => v,
        FusedWitnessOracle::Direct(fw) => {
            direct_witness_eval(fw, &sumcheck_challenges, col_bits, ring_bits)?
        }
    };
    let eq_val_cr = EqPolynomial::mle(&r_stage1, &sumcheck_challenges);
    let cr_expected =
        eq_val_cr * (batching_coeff * final_w_eval * (final_w_eval + F::one()) + final_w_eval);
    ...
    MEval: FnOnce(&[F]) -> Result<F, HachiError>,
{
    ...
    let round_result = {
        let _sumcheck_span = tracing::info_span!("stage2_sumcheck_batched_with_carry").entered();
        verify_batched_sumcheck_rounds::<F, T, F, _>(
            stage2_sumcheck,
            vec![&thin_cr, &thin_carry],
            transcript,
            |tr| tr.challenge_scalar(CHALLENGE_SUMCHECK_ROUND),
        )?
    };
    ...
    let r_final: Vec<F> = round_result.r_sumcheck[carry_offset..].to_vec();
...
mod tests {
    ...
    /// Serialize tests that assert on the `CARRY_CLOSURE_*` counters so
    /// concurrent test threads don't race on the shared atomics.
    pub(crate) static CARRY_CLOSURE_TEST_MUTEX: std::sync::Mutex<()> = std::sync::Mutex::new(());
    ...
    #[test]
    fn verify_passes_for_consistent_opening_fused_mode() {
        let alpha = D.trailing_zeros() as usize;
        let layout = Cfg::commitment_layout(16).unwrap();
        let num_vars = layout.m_vars + layout.r_vars + alpha;
    ...
    #[test]
    fn verify_passes_for_consistent_opening_fused_carry_setup_claim() {
        // Hold the carry-closure counter mutex so
        // `fused_carry_setup_claim_consumes_carry_at_level_one` doesn't
        // double-count our invocations when both tests run in parallel.
        let _guard = CARRY_CLOSURE_TEST_MUTEX
            .lock()
        ...
        let mut prover_transcript =
            Blake2bTranscript::<F>::new(b"test/prove/fused-carry-setup-claim");
        let proof = <Scheme as CommitmentScheme<F, D>>::prove(
            &setup,
            &poly,
            &opening_point,
        ...
        assert!(
            result.is_ok(),
            "fused + carry-setup-claim roundtrip failed: {result:?}"
        );
    }

    /// Verifies that the L=1 setup-claim carry closure actually fires on the
    /// fused + carry-setup-claim path for a `level_d == D` config, on both
    /// the prover and the verifier side. Uses the test-only
    /// [`CARRY_CLOSURE_PROVE_CALLS`] / [`CARRY_CLOSURE_VERIFY_CALLS`]
    /// counters to distinguish the carry-closure path from the plain fused
    /// fallback (no setup claim).
    #[test]
    fn fused_carry_setup_claim_consumes_carry_at_level_one() {
        use std::sync::atomic::Ordering;

        // Serialize against any other test that inspects the shared
        // `CARRY_CLOSURE_*` counters, otherwise parallel test threads
        // double-count carry-closure invocations.
        let _guard = CARRY_CLOSURE_TEST_MUTEX
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
