...
const D: usize = 64;
const GREYHOUND_POLY_VARS: usize = 25;
const RING_BYTES: usize = std::mem::size_of::<CyclotomicRing<F, D>>();
const TARGET_WITNESS_BYTES: usize = 150 * 1024;
...
fn derive_greyhound_instance(
    ...
    let (eval_point, eval_value) = {
        ...
        (eval_point, eval_ring.coefficients()[0])
    };
...
/// Run the recursive Labrador prover until the final witness is ≤ TARGET_WITNESS_BYTES,
/// reporting parameters at each level for comparison with the C reference.
fn recursive_prove_until_target(
    ...
    while level_idx < max_levels {
        ...
        if cur_witness.rows().len() <= 1 {
            if verbose {
                eprintln!("  ==> single row witness, cannot fold further.");
            }
            break;
        }

        let is_tail = wb <= 2 * TARGET_WITNESS_BYTES;
        let plan = plan_fold::<F, D>(&cur_witness, is_tail).expect("plan_fold failed");
        let cfg = plan.config;
        ...
        if verbose {
            let m = cfg.fu * rr * cfg.kappa + cfg.fu * (rr * rr + rr) / 2;
            eprintln!();
            eprintln!(
                "  L{level_idx} config: f={} b={} fu={} bu={} kappa={} kappa1={} tail={}",
                cfg.f, cfg.b, cfg.fu, cfg.bu, cfg.kappa, cfg.kappa1, cfg.tail
            );
            eprintln!(
...
fn report_recursive(poly_vars: usize) {
    let (gh_proof, witness, statement, comkey_seed) = derive_greyhound_instance(poly_vars);

    eprintln!("=== Labrador recursive prover report (poly_vars={poly_vars}) ===");
    eprintln!(
    ...
    eprintln!("  constraints     : {}", statement.constraints.len());
    eprintln!("  target witness  : {} KB", TARGET_WITNESS_BYTES / 1024);
    eprintln!();
    ...
    let final_elems = witness_ring_elems(&proof.final_opening_witness);
    let final_bytes = witness_bytes(&proof.final_opening_witness);
    let proof_bytes = proof.size();
    let levels_bytes: usize = proof.levels.iter().map(|l| l.size()).sum();

    eprintln!();
    eprintln!("  --- summary ---");
    eprintln!("  fold levels          : {}", proof.levels.len());
    eprintln!(
    ...
    let t0 = Instant::now();
    verify(&statement, &proof, &comkey_seed, &mut verify_transcript).expect("verification failed");
...
fn bench_labrador_recursive_prove(c: &mut Criterion) {
    let (_gh_proof, witness, statement, comkey_seed) =
        derive_greyhound_instance(GREYHOUND_POLY_VARS);

    report_recursive(GREYHOUND_POLY_VARS);

    let mut group = c.benchmark_group(format!(
        "labrador/recursive/greyhound_{GREYHOUND_POLY_VARS}v"
    ));
...
criterion_group!(labrador_recursive_benches, bench_labrador_recursive_prove);
