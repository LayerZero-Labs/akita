...
use hachi_pcs::protocol::labrador::aggregation::aggregate_jl_functions;
...
fn bench_aggregate_jl_functions_for_field<F: FieldCore + CanonicalField + 'static>(
    c: &mut Criterion,
    field_name: &str,
) {
    let cols = BENCH_COLS;
    ...
    c.bench_function(
        &format!("labrador/aggregate_jl_functions/{field_name}"),
        |b| {
            b.iter(|| {
                let got =
                    aggregate_jl_functions::<F, D>(black_box(&matrix), black_box(&omega)).unwrap();
                black_box(got);
            })
        },
    );
...
fn bench_aggregate_jl_functions_fp32(c: &mut Criterion) {
    bench_aggregate_jl_functions_for_field::<Pow2Offset32Field>(c, "fp32");
}

fn bench_aggregate_jl_functions_fp64(c: &mut Criterion) {
    type F64 = Pow2Offset64Field;
    bench_aggregate_jl_functions_for_field::<F64>(c, "fp64");
}

fn bench_aggregate_jl_functions_fp128(c: &mut Criterion) {
    type F128 = Prime128M13M4P0;
    bench_aggregate_jl_functions_for_field::<F128>(c, "fp128");
}

criterion_group!(
    labrador_jl_aggregation,
    bench_aggregate_jl_functions_fp32,
    bench_aggregate_jl_functions_fp64,
    bench_aggregate_jl_functions_fp128
);
