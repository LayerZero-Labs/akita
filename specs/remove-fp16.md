...
- The Q16 SIS family (`SisModulusProfileId::Q16`) and all Q16 floor table rows and wire-format tags.
...
- **Instance descriptor decoding is strict.**
  Removing `SisModulusProfileId::Q16` means the descriptor tag previously used for Q16 must be rejected explicitly.
...
- **Surviving wire tags are preserved.**
  The `SisModulusProfileId` tag values for the kept families stay unchanged: `Q32 = 0`, `Q64 = 1`, `Q128 = 2` in `crates/akita-types/src/descriptor_bytes.rs` (`sis_modulus_profile_tag`) and the matching decoder in `crates/akita-types/src/instance_descriptor.rs`.
...
- [x] `SisModulusProfileId::Q16` does not exist, and there are no Q16 SIS floor rows shipped in code.
- [x] No shipped or profile preset resolves to `SisModulusProfileId::Q16`; the only families selected are Q32/Q64/Q128.
...
```bash
rg -n --glob '!specs/**' 'Fp16|Prime16Offset99|fp16\b|Fp16Packing|FoldMatrixFp16|RingSubfieldFp8Fp16|SisModulusProfileId::Q16|\bQ16_PRIMES\b|\bQ16_NUM_PRIMES\b|\bQ16_MODULUS\b|\bq16_garner\b'
```
...
- `akita-types`
  - Removes `SisModulusProfileId::Q16` and every match arm over it, including the `ceil_supported_collision` `(Q16, d)` rows in `crates/akita-types/src/sis/ajtai_key.rs`.
  ...
  - Updates descriptor encoding (`sis_modulus_profile_tag`) and decoding so the removed Q16 tag `3` is rejected, while `Q32 = 0`, `Q64 = 1`, `Q128 = 2` stay unchanged.

- `akita-config`
  - Removes the `SisModulusProfileId::Q16` arm from the small-field capacity test (`crates/akita-config/src/proof_optimized/tests.rs`).
...
4. Remove `SisModulusProfileId::Q16` plus all descriptor-tag handling and SIS table rows in `akita-types`, and the dependent `akita-config` test arm.
5. Run the acceptance commands and `rg` audit.

Risks to watch:

- Verifier-reachable tag decoding must return `SerializationError`, not panic.
- Removing `SisModulusProfileId::Q16` is a wire-format break, so tests must assert explicit rejection, not silent acceptance.
