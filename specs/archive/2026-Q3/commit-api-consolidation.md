# Historical spec: Commit API consolidation

> **Historical design record.** The migration this spec proposes has landed.
> Its API surface is now documented in
> [`book/src/usage/commitment-api.md`](../../../book/src/usage/commitment-api.md).
> Names and signatures below describe the design as proposed, not the shipped
> surface: `PriorGroupProfiles` is non-empty by construction, both grouped
> `GroupContext` constructors are infallible, and batch assembly derives the
> prior prefix rather than taking it.
