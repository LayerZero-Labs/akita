...
Structural divergence at the same `(preset, nuposition_index_bits, incidence)` key is therefore
...
- `generated_families_stay_within_audited_sis_widths` (zk only) spot-checks every shipped
  family at `nuposition_index_bits ∈ {8,16,28,30}` against `min_secure_rank` using stored
