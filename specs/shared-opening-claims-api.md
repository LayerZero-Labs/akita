...
| `PolynomialGroupLayout` | per-group `(nuposition_index_bits, num_polynomials)` layout; also the schedule lookup group key |
...
| `OpeningBatchLimits` | removed — `OpeningClaims::validate(&AkitaSetupSeed)` reads `max_nuposition_index_bits` / `max_num_batched_polys` directly |
...
| **`OpeningClaimsLayout`** | per-group `nuposition_index_bits`, `num_polynomials` only | setup sizing, schedule lookup, planner, commit-param preview, layout tests |
...
`nuposition_index_bits` field** — each group's active variable count lives on
`PolynomialGroupLayout`, and the batch exposes `max_nuposition_index_bits()` when a single
scalar is needed.

```rust
impl OpeningClaimsLayout {
    /// Single full-point group (production default).
    pub fn new(nuposition_index_bits: usize, num_total_polynomials: usize) -> Result<Self, AkitaError>;

    /// Multi-group structure (planner / multi-group specs).
    /// Every group receives the same `nuposition_index_bits` (shared padded point).
    ...
    pub fn max_nuposition_index_bits(&self) -> usize {
        self.groups()
            .iter()
            .map(PolynomialGroupLayout::nuposition_index_bits)
            .max()
            .unwrap_or(0)
    }
...
impl PolynomialGroupLayout {
    ...
    pub const fn new(nuposition_index_bits: usize, num_polynomials: usize) -> Self;

    /// Runtime validation (non-const callers).
    pub fn validate(self) -> Result<(), AkitaError>;

    pub const fn nuposition_index_bits(self) -> usize;
    pub const fn num_polynomials(self) -> usize;

    /// Scalar default: one polynomial at `nuposition_index_bits`.
    pub const fn singleton(nuposition_index_bits: usize) -> Self;
}
```

Default per-group geometry: `PolynomialGroupLayout::new(nuposition_index_bits, k)`.
...
| `shape.nuposition_index_bits()` | `layout.max_nuposition_index_bits()` |
...
AkitaScheduleLookupKey {
    final_group: PolynomialGroupLayout::new(nuposition_index_bits, num_polynomials)?,
    precommitteds: vec![],
}
```

**Test migration:** `group_batch_key_allows_precommitted_nuposition_index_bits_above_half_main`
...
| `const fn new(nuposition_index_bits, num_polynomials)` on `PolynomialGroupLayout` | Emitted by offline table generation |
...
| Location | Today | Next |
|----------|-------|------|
| `prepare_batched_commit_inputs` | returns `OpeningBatchShape` | returns `OpeningClaimsLayout` from poly count + padded `max_nuposition_index_bits` |
...
| Descriptor digest routing tests | `OpeningBatchShape::from_groups` with custom `point_vars` | claims-side `PolynomialGroupClaims` with custom `point_vars`; layout uses derived `nuposition_index_bits` |
| Catalog / planner unit tests | shape / `CommitmentGroupScheduleKey` | layout / `PolynomialGroupLayout` |
| Grouped schedule validation | allows precommitted above half main | accepts arbitrary per-group `nuposition_index_bits` supported by setup/schedule |
...
`AkitaSetupSeed` (`max_nuposition_index_bits`, `max_num_batched_polys`) remains the stored
**capacity envelope**; `OpeningClaimsLayout::from_setup_seed` is the typed view
for schedule/setup code that today synthesizes `OpeningBatchShape::new(seed.max_nuposition_index_bits, seed.max_num_batched_polys)`.
...
#### 14. `point_vars` on claims only; layout uses `nuposition_index_bits`

**Decision:** `PolynomialGroupLayout` stores **`nuposition_index_bits: usize`**, not
...
derives each group's `nuposition_index_bits` from its `point_vars` (active coordinate count).
...
| Batch max vars | `layout.max_nuposition_index_bits()` is the batch-wide padded point length; equals `opening_claims.nuposition_index_bits()` for valid claims |
...
| Layout/claims consistency | `opening_claims.layout()` matches group sizes and per-group `nuposition_index_bits` derived from claims routing |
...
| Grouped-root sizing | Precommitted `nuposition_index_bits` are independent of final-group `nuposition_index_bits`; setup/schedule capacity decides support |
...
- [ ] `OpeningClaimsLayout` has no batch-level `nuposition_index_bits`; exposes `max_nuposition_index_bits()` and `groups()`.
...
- Update schedule tests: precommitted `nuposition_index_bits` above half of `final_group.nuposition_index_bits` and larger than the final group are accepted when setup/schedule capacity supports them.
...
```text
   ...
   └─ group_polys(g) / flat_polys()
                          ...
                          ▼
                 OpeningClaimsLayout
                 └─ groups: [PolynomialGroupLayout { nuposition_index_bits, num_polynomials }, ...]
                          │
        ┌─────────────────┼───────────────────────────┐
        ▼                 ▼                            ▼
...
impl<'a, F, C> OpeningClaims<'a, F, C> {
    ...
    /// envelope (`seed.max_nuposition_index_bits`, `seed.max_num_batched_polys`). Returns
...
impl OpeningClaimsLayout {
    pub fn new(nuposition_index_bits: usize, num_total_polynomials: usize) -> Result<Self, AkitaError>;
    pub fn from_group_sizes(nuposition_index_bits: usize, polynomials_per_group: &[usize]) -> Result<Self, AkitaError>;
    pub fn from_groups(groups: Vec<PolynomialGroupLayout>) -> Result<Self, AkitaError>;
    pub fn from_setup_seed(seed: &AkitaSetupSeed) -> Result<Self, AkitaError>;
...
impl<'a, PointF, P, CommitF, const D: usize> ProverOpeningData<'a, PointF, P, CommitF, D> {
    ...
    pub fn nuposition_index_bits<PolyF>(&self) -> Result<usize, AkitaError>
