...
5. Each successful case must emit setup, commit, prove, verify, proof-size,
   ...
   failure.
   Terminal folded levels also report the terminal relation mode, relation-tag
   bytes, optional stage-2 bytes, and final-witness bytes so
   `DirectRingRelations` is distinguishable from a missing or root-direct
   stage-2 field.
...
Terminal direct mode adds a second terminal proof-size shape to this accounting:
`RingSwitchSumcheck` terminals include the one-byte relation tag, terminal
stage-2 sumcheck bytes, and the quotient-bearing final witness, while
`DirectRingRelations` terminals include the same tag, zero terminal stage-2
bytes, and a final witness with the `r_hat` quotient segment omitted. Reports
therefore surface the relation mode and per-terminal fields directly instead of
asking readers to infer direct mode from a zero-sized stage-2 column.
