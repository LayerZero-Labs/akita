...
- **Why the gap remains**: the planner's objective only charges wire bytes. The cleartext MLE check at a direct-terminating L+1 has no wire cost (verifier recomputes the MLE locally from public S), so direct always looks cheaper. The cascade's actual benefit is verifier wall-clock + setup-precompute storage, neither of which is in the current objective.
...
- **Production readiness**: NICE-TO-HAVE. Force-routing is the canonical opt-in today; gap closure (option (a) below) would let the DP discover the cascade unprompted.
- **Recommended fix**: option (a) extend the planner objective to charge a notional verifier-wall-clock cost for the cleartext MLE discharge proportional to `|S_remaining|`, so the cascade's setup-precompute amortisation wins at large NV; option (b) leave the force-routing gates as the canonical opt-in (current state).
