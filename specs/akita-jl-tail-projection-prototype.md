...
- **No overflow on the integer path.** Centering uses balanced representatives in `[-(q-1)/2, (q-1)/2]`; centering and coordinate accumulation use `i64` (JL inputs are small balanced digits, so `i64` is ample) and return an error if a centered coefficient or accumulated coordinate would escape `i64`. Do not silently saturate. A non-digit input (e.g. a full-magnitude fp128 element) is therefore rejected, which is correct: it is not a JL input. The squared-norm reduction is the one place a wider accumulator is warranted (squaring an `i64`), so `l2_norm_sq_checked` accumulates in `u128`; it is `O(n_rows)` and off the hot projection path. Protected by small-digit, oversized-rejection, and norm-bound tests.
...
- `project<F>(&self, coeffs: &[F]) -> Result<JlImage, AkitaError>` (`F: FieldCore + CanonicalField`): centers each coefficient to its balanced representative and computes the exact integer product over `i64` coordinates. JL is only ever applied to small balanced digits, so `i64` holds both the centered coefficients and the accumulated coordinates with room to spare; there is no `i128` in this path. The modulus is recovered for `CanonicalField` as `(-F::one()).to_canonical_u128() + 1`; if a clearer modulus helper is added later, use it instead. Accumulation is checked, so a non-digit input whose centered value or accumulated coordinate would escape `i64` (e.g. a full-magnitude fp128 element) is rejected rather than wrapped or saturated. Returns an error on overflow or on a `coeffs.len() != cols` shape mismatch.
