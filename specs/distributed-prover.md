...
- `z_len_i = num_digits_fold · num_digits_commit · positions_per_block` (replicated, full),
...
let blocks_per_chunk = lp.live_block_count / num_chunks;   // B_loc, power of two
...
| `num_chunks > 1` and `lp.live_block_count % num_chunks != 0` | `InvalidSetup` |
...
the same preset for `W ∈ {1,2,4,8}`, `positions_per_block` pow2 (root) and dense (recursive).
...
- [ ] The modified-relation proof verifies under the chunked verifier for
  `W ∈ {2,4,8}` (pow2 and dense `positions_per_block`); `W = 1` matches the legacy proof.
...
6. **Determinism** and **no-panic negatives** (bad `num_chunks`,
   `num_chunks ∤ live_block_count`, zk+chunked).
