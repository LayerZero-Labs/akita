...
- Stage 1 has one job: prove the exact norm of semantic `Z`.
- Stage 2 proves the exact norms of literal `Ehat` and `That` directly inside
  the ordinary witness-domain sumcheck.

`Z` needs a separate norm sumcheck because it is a virtual recomposition of
committed `Zhat`. `Ehat` and `That` need no virtualization because they are
already literal intervals of the Stage-2 witness.
...
|---|---:|---:|---|---|
| semantic `Z` | `S_Z` | `C_Z` | `A` | Stage 1 norm sumcheck; Stage 2 virtualization |
...
`Zhat` is an existential algebraic preimage. The accepted semantic response is

```text
Z = ctr_q(G_b Zhat).
```

The protocol constrains the centered field vector `Z`, not an unreduced
integer gadget sum. A noncanonical `Zhat` is acceptable when it satisfies the
field relation and all other witness constraints.
...
| semantic `Z` | `ProjZ` | Stage 1 | `A` | `Z = ctr_q(G_b Zhat)` in Stage 2 |
...
statement: there is a committed `Zhat` whose field recomposition is the
accepted semantic `Z`. The `A` reduction consumes the exact norm of that
semantic `Z`, not the norm or uniqueness of `Zhat`. The honest prover MAY keep
using the canonical balanced decomposition, but the verifier MUST NOT require
canonicality after this cutover.

The distinction is load-bearing. The verifier and security inventory MUST use

```text
Z(u) = ctr_q(sum_h b^h Zhat(u,h) mod q),
```

not the unreduced integer gadget sum.
...
Stage 1
  exact integer S_Z
  semantic-Z norm sumcheck
  terminal semantic-Z evaluations
...
6. Send S_Z and run Stage 1, whose only relation is the exact Z norm.
...
```text
semantic Z --------------------> Z stem ----> Z tail ----> clear v_Z
...
source MLE claims for semantic `Z`, literal `Ehat`, and literal `That`.
...
the committed `Zhat` planes:

```text
L(Z) = sum_u L_u sum_h b^h W(pi_Z(u,h)).
```
...
For accepted challenge multiplication-operator norm `Gamma`:

```text
eta_A,2^2 = 64 Gamma^2 C_Z.
```
...
inside Stage 1, whose only job is exact semantic-`Z` norm certification. Their
terminal semantic claims are virtualized to the corresponding `Zhat` intervals
in Stage 2.
...
1. computes honest role energies for `Z`, `Ehat`, and `That`;
...
|---|---|---|
| JL sources | one `ProjZ`, one aligned `ProjET` | same roles, split only the roles that fail no-wrap |
...
- Refactor Stage 1 to contain only semantic-`Z` norm certification.
...
- Add exact `Z` shard norms to Stage 1.
...
- [ ] Stage 1 contains only exact semantic-`Z` norm certification.
...
- [ ] Compression F/H remains negative binary under an independent equality
      point.
- [ ] `C_Z`, `C_E`, and `C_T` independently price `A`, `D`, and `B`.
