...
```text
z_coords       = num_z_segments * positions_per_block * num_digits_commit * D   (Golomb integers; one per base-field slot in folded z)
e_field_elems  = live_block_count * num_w_vectors * D                       (RawField; one ring element per block → D coeffs)
t_field_elems  = n_a * live_block_count * num_t_vectors * D                   (RawField)
