...
- [ ] Profile shows the expected per-level shrink: `AKITA_MODE=onehot_fp128_mixed_dim AKITA_NUM_VARS=32 cargo run --release --example profile` reports `y_ring_bytes = 0` at every level and total proof size reduced by the predicted amount.
