...
Thus the evaluator is logarithmic in the padded setup domain for a fixed tensor
description and bounded carry state. It is not unconditionally `O(log N)` in
all geometry parameters: group count, family count, dense fold/row axes, and
carry-state multiplicity remain explicit work. The required invariant is that
it never performs an `O(N)` padded setup-index scan or allocation.
...
- [ ] For non-power-of-two `R`, preprocessing commits exactly
  ```
  `shared_setup[..N * d0]` and the recursive opening source reconstructs
  byte-for-byte the same polynomial.
  ```
- [ ] Setup generation rejects a requested full prefix when fewer than
  ```
  `N * d0` actual shared setup coefficients are available.
  ```
- [ ] The dense Boolean Stage-3 claim with `S_full` and zero-tail `W_R` equals
  ```
  the existing natural-prefix setup contribution.
  ```
- [ ] The Stage-3 prover and verifier agree on the full-prefix terminal
  ```
  relation for random non-Boolean challenges.
  ```
- [ ] `setup_prefix_eval` equals a dense oracle evaluation of `S_full`, including
  ```
...
- [ ] The successor grouped opening accepts the honest carried full-prefix
  ...
  or group order.
  ```
- [ ] The compact setup-weight evaluator equals the MLE of a dense length-`N`
  ```
  weight vector whose tail is explicitly zero.
  ```
- [ ] A scalar truncated shifted-equality fixture covers every `R` for small
  ```
  domains and matches dense
  `sum_{i<R} chi_i(r) * chi_i(z)` evaluations.
  ```
- [ ] Arbitrary-length affine stream tests include lengths `1`, `N`, `N-1`, and
  ```
  lengths with multiple binary blocks such as `13 = 8 + 4 + 1`.
  ```
- [ ] Shifted/strided tests exercise carries on both equality addresses and
  ```
  confirm that no rounded-up stream coordinates contribute.
  ```
- [ ] Mixed-ring, multigroup, and multichunk Stage-3 fixtures retain dense versus
  ```
...
- [ ] Transcript logging shows identical prover/verifier events and binds the
  ...
  ```
- [ ] No verifier path allocates `O(N)` setup-index weights or equality values.
