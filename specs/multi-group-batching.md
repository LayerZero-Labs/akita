...
impl OpeningClaimsLayout {
    pub fn new(nuposition_index_bits: usize, num_polys: usize) -> Result<Self, AkitaError>;
    ...
    pub fn max_nuposition_index_bits(&self) -> usize;
...
impl<'a, F: Clone, C> OpeningClaims<'a, F, C> {
    ...
    pub fn nuposition_index_bits(&self) -> usize;
    pub fn num_total_polynomials(&self) -> usize;
    pub fn num_groups(&self) -> usize;
    pub fn group_sizes(&self) -> Vec<usize>;
}
...
```text
nuposition_index_bits        = committed/opened arity for this group
num_polynomials = K_g
```
...
```rust
AkitaScheduleLookupKey::single(PolynomialGroupLayout::new(nuposition_index_bits, K))
```
...
(`num_polynomials in {1, 4}` today), sets each precommitted group's `nuposition_index_bits`
to `main.nuposition_index_bits / 2`, and emits one- or two-precommit patterns:
...
nuposition_index_bits in supported family ranges
```
...
```text
sum_g K_g * 2^{nuposition_index_bits_g}
```
...
```text
multi-group proof bytes >= scalar [4] proof bytes for the same nuposition_index_bits
...
2. **Setup capacity:** keep the public setup surface on `max_nuposition_index_bits`,
