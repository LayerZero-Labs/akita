...
14. `MAX_CRT_RING_DEGREE = 256`; `D in {32, 64, 128, 256}` are the only supported
    ...
    `2048 | (p - 1)`.
    `D = 512` and `D = 1024` are removed from `SUPPORTED_COMMITMENT_RING_DIMS`, the
...
- [ ] `select_crt_ntt_params` dispatches `q <= Q16_MODULUS` to `Q16` before the
      ...
      Q64").
- [ ] `D = 512` / `D = 1024` are removed from `SUPPORTED_COMMITMENT_RING_DIMS`, the
...
1. Lower `MAX_CRT_RING_DEGREE` to `256`; remove `D = 512` / `D = 1024` from
   `SUPPORTED_COMMITMENT_RING_DIMS`, the dispatch macros, the fp16/fp32 `D512*` presets, and
