...
- `nuposition_index_bits`: number of variables in every opening point.
...
`max_nuposition_index_bits` is not a scheduler/planner key dimension after preprocessing.
...
`nuposition_index_bits`, and in practice all groups in the same batch use one root layout.
...
The current implementation has already moved `nuposition_index_bits` into the schedule key
and removed the separate scheduler `max_nuposition_index_bits` dimension. The next step is to
...
Do not reintroduce `max_nuposition_index_bits` here. Generated schedules are keyed by the
actual root problem shape, not by setup capacity. A setup that supports
`max_nuposition_index_bits = N` must instead size its matrix envelope over all generated or
planner-supported runtime shapes with `nuposition_index_bits <= N`.
...
`max_nuposition_index_bits` remains a setup-capacity concept:

- `AkitaSetupSeed::max_nuposition_index_bits` bounds accepted commitment/proof inputs.
- `ClaimIncidence::validate` and batched input validation reject claims whose
  actual `nuposition_index_bits` exceeds setup capacity.
- Setup matrix sizing must conservatively cover every actual runtime shape the
  setup may serve.

Because schedule lookup no longer includes `max_nuposition_index_bits`, setup preprocessing
must not size only the all-up shape. It must take the maximum over:

```text
1 <= nuposition_index_bits' <= setup.max_nuposition_index_bits
...
runtime `nuposition_index_bits` or differently multi-group batch may select a schedule with larger
...
- `find_optimal_schedule` accepts the schedule/profile key without any
  `max_nuposition_index_bits` dimension.
...
1. Remove scheduler/planner `max_nuposition_index_bits` from lookup keys, generated keys, and
   schedule inputs. Done.
2. Ensure setup preprocessing sizes the matrix envelope over all actual
   `nuposition_index_bits <= setup.max_nuposition_index_bits`. Done.
