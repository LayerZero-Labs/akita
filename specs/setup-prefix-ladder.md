...
```text
ring_slots = num_blocks * block_len
```

`setup_prefix_level_params(next_params, N_prefix, D_setup)` searches for a
power-of-two `num_blocks` divisor of `ring_slots` such that the normal Akita
commitment dimensions fit:

```text
block_len * num_digits_commit <= a_key.col_len()
num_blocks * a_key.row_len() * num_digits_open <= b_key.col_len()
```
