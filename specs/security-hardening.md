...
- [x] `LevelParams` validation rejects invalid verifier layouts, including zero or non-power-of-two ring dimensions, unsupported `log_basis`, zero `live_block_count`, inconsistent `block_index_bits`, zero `positions_per_block`, overflowing row/column widths, and malformed digit depths.
...
- **Level layout shape:** ring-switch and folded-root verification assume valid `LevelParams`.
  Addressed by shared verifier layout guards for `ring_dimension`, `log_basis`, `live_block_count`, `positions_per_block`, `position_index_bits`, `block_index_bits`, digit depths, row counts, and derived widths.
