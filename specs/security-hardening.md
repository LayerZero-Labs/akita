...
- [x] `LevelParams` validation rejects invalid verifier layouts, including zero or non-power-of-two ring dimensions, unsupported `log_basis`, zero `num_live_blocks`, inconsistent `block_index_bits`, zero `num_positions_per_block`, overflowing row/column widths, and malformed digit depths.
- [x] Ring-switch preparation rejects inconsistent opening-point, challenge, gamma, group-routing, block, and row-weight shapes before constructing `RelationWeightEvaluator`.
...
- **Level layout shape:** ring-switch and folded-root verification assume valid `LevelParams`.
  Addressed by shared verifier layout guards for `ring_dimension`, `log_basis`, `num_live_blocks`, `num_positions_per_block`, `position_index_bits`, `block_index_bits`, digit depths, row counts, and derived widths.
- **Ring-switch prepared state:** `RelationWeightEvaluator::eval_flat_at_point` and `build_relation_weight_events` assume preparation already validated challenge lengths, block summaries, opening-point lengths, `eq_tau1` rows, group routing, and setup capacity.
  Addressed in `prepare_relation_weight_evaluator`, `ring_switch_verifier`, and the row-eval entry checks, without fallback arithmetic paths.
