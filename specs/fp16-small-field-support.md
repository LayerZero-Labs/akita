...
4. Planner and generated schedules
   ...
   - D32 one-hot nv32 and dense singleton nv26/nv27 reproduce the dry-run estimates produced by the planner dry-run binary (see §"Current Proof-Size Targets") exactly. Any divergence is documented as an intentional estimator/table change.
...
These are planner dry-run estimates produced by the planner dry-run binary scaffolded for this work (currently `crates/akita-planner/src/bin/small-field-dryrun.rs`, to be productized under a stable name as part of Phase 0). The binary expresses the planner formulas in terms of base-field/challenge-field bit widths and consumes SIS floors from the CSV generated by `scripts/gen_sis_table.py --format csv`, so the numbers below are reproducible without checking in the `Fp16` field type first.

Reproducer (subject to the final binary name):

```bash
sage -python scripts/gen_sis_table.py --family q16 --max-rank 20 --format csv \
  > target/sis-dryrun/fp16.csv
cargo run -p akita-planner --release --bin small-field-dryrun -- \
  --field fp16 --d 32 --workload onehot --num-vars 32
cargo run -p akita-planner --release --bin small-field-dryrun -- \
  --field fp16 --d 32 --workload dense  --num-vars 26
```

Estimates (assume tight 2-byte fp16 serialization):
...
The same reproducer must reproduce the existing fp32 numbers to the byte before any fp16 floor lands, so the planner-side formulas (including length-prefix accounting and the shared root EOR partial helper from acceptance criterion 6) are validated against a known-good baseline.
...
All proof-size estimates committed to this spec come from the planner dry-run binary (productized in Phase 0). No private drafting note is part of the spec's reproducibility surface.
...
- Productize the planner dry-run binary. The scaffolded binary currently lives at `crates/akita-planner/src/bin/small-field-dryrun.rs` and is bit-parametric in base/challenge widths. Move it to a stable name and stage it (no `Fp16` type required to run it). The binary becomes the source of truth for §"Current Proof-Size Targets".
...
- The productized planner dry-run binary builds and reproduces the fp32 D32 onehot nv32 and dense singleton nv26 numbers to the byte.
...
- Confirm the planner dry-run binary productized in Phase 0 is the only proof-size oracle the spec depends on; remove any remaining local-only dry-run scaffolding.
...
- `crates/akita-planner/src/bin/small-field-dryrun.rs` (productized in Phase 0; final path TBD)
