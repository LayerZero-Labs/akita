...
- **Mixed `natural_nuposition_index_bits` per slot in one batch.** Future work if a caller exists;
...
- `root_fold_shape` is evaluated with `AkitaScheduleInputs { nuposition_index_bits: key.nuposition_index_bits,
  level: 0, current_w_len: 1usize.checked_shl(key.nuposition_index_bits as u32)? }`.
...
- `key_count` and `key_digest` cover the sorted generated keys, including `nuposition_index_bits`,
...
Production folded path uses `OpeningClaimsLayout::new(padded_nuposition_index_bits, num_polys)?`
...
- `nuposition_index_bits` in `[min_nuposition_index_bits, max_nuposition_index_bits]`
...
may build those keys from `nuposition_index_bits: 18..=32` and `num_polys: [1, 38]`, but the emitter
...
3. Generate `jolt_fp128_d64_onehot.rs` with explicit keys:
   `nuposition_index_bits` envelope × `num_polys` e.g. `[1, 38]`).
