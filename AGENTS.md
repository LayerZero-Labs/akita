...
- `akita-prover` — commitment, proving, setup expansion, recursive/ring-switch witnesses, polynomial backends. **Depends on `akita-config`** and is directly `<Cfg>`-generic: `batched_prove::<Cfg, T, P, C, O, TS, R, D>` (in `protocol::core::prove`) takes `&impl LevelProveStacks` and routes commit / opening / tensor / ring-switch through `ProverComputeStack` + `OperationCtx`; `commit` / `batched_commit` take `&UniformProverStack` (or per-cluster `OperationCtx`); `commit_next_w` and `prove_suffix` call cluster backends directly. Root polys expose `RootCommitSource` / `RootOpeningSource` / `RootTensorSource`; backends implement source-typed kernels (`RootCommitKernel`, `OpeningFoldKernel`, `TensorProjectionKernel`, `RingSwitchRelationKernel`, …). Calls `Cfg::…` and `bind_transcript_instance_descriptor` directly with no `_with_policy` closures.
...
- `DensePoly`, `OneHotPoly`, `MultilinearPolynomial`, `RecursiveWitnessFlat` — built-in polynomial representations; custom roots implement `RootCommitSource` / `RootOpeningSource` / `RootTensorSource` (+ `DirectRootWitnessSource` when root-direct applies)
- `ProverComputeStack`, `LevelProveStacks`, `OperationCtx` — per-fold stack selection and per-cluster backend routing (commit / opening / tensor / ring-switch)
