...
- `PlanPolicy` — value-typed inputs to `akita_types::schedule_plan_from_table` (table materialization). Planner search is `<Cfg>`-generic — no separate `SearchOptions`-shaped input
- `PlannerCfg<Cfg>` — test-only `Cfg` wrapper in `akita_planner::test_utils` (gated by the `test-utils` Cargo feature) that overrides the `CommitmentConfig` parameter accessors to route schedule-table misses through `find_schedule::<Self>`. Production presets do not need this wrapper and never link it
