...
## Sumcheck table kernels

Dense sumcheck tables use one canonical coefficient-first representation. The
CPU plan selects scalar, NEON, AVX2, or AVX-512 arithmetic once per operation.
With one worker, fused packed traversals avoid extra table passes. With several
workers, folds and product rounds partition the same coefficient slabs into
aligned ranges and reduce exact partial sums.

The architecture and extension-opening details live in
[`specs/sumcheck-kernel-architecture.md`](../../../specs/sumcheck-kernel-architecture.md)
and
[`specs/eor-sumcheck-prover-acceleration.md`](../../../specs/eor-sumcheck-prover-acceleration.md).
