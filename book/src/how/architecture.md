...
3. **Commit.** The role-aware `commit` entry point (in `akita-prover`, orchestrated by `akita-pcs`) produces one committed polynomial group using `GroupPosition::{Independent, Final}`. An independent S commitment may later be supplied as a prior group.
