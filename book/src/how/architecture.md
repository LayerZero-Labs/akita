...
3. **Commit.** The context-aware `commit` entry point (in `akita-prover`, orchestrated by `akita-pcs`) produces one committed polynomial group using `GroupContext`. Scheduler mode selects S without prior groups or the exact G row with prior groups; explicit mode validates caller-supplied root parameters. An S commitment may later be supplied as a prior group.
