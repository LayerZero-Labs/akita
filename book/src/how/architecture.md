...
1. **Preset selection.** The caller picks a `CommitmentConfig` preset (`fp31` / `fp32` / `fp63` / `fp64` / `fp128` families). `CommitmentConfig::resolve_catalog_row_for_key` resolves one complete row from the shipped catalog; planner search remains offline. The preset also fixes the protocol geometry: when the claim field coincides with the coefficient field (`EXT_DEGREE == 1`, today's `fp128` families) the fold path never runs extension-opening reduction; when claims live in a proper extension (`fp31` / `fp32` / `fp63` / `fp64`), root EOR follows `akita_types::root_tensor_projection_enabled` and suffix EOR follows `EXT_DEGREE > 1`. See [Fold path and field geometry](./proving/fold-path.md).
