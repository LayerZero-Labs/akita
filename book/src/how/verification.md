...
3. **Replay the structural folds** in `protocol/core`: the root fold followed by
   every recursive fold, using the parameters in the resolved `FoldSchedule`.
...
4. Strengthen validation at deserialization, setup construction, schedule expansion, and verifier API entry points rather than sprinkling checks through hot loops.
