...
2. **Resolve the schedule** the prover used (`CommitmentConfig::runtime_schedule`), validating `nuposition_bits` against setup capacity before any DP fallback.
...
The verifier must validate `key.nuposition_bits` against setup capacity **before**
