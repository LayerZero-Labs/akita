...
In a standard in-memory prover, the encoded word and hash tree require memory
that grows linearly with the polynomial size. A prover can reduce RAM by
recomputing data or moving it to external storage, but then pays more work or
disk traffic.
