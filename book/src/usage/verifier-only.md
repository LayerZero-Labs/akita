...
(`crates/akita-verifier/src/lib.rs`). `resolve_group_batch_schedule` is reached
transitively via `akita-config` and is described here only to explain the
strictness behavior; consumer code does not need to name `akita-schedules`
directly. The strict resolution that *is* verifier-reachable rejects
unsupported requests with `AkitaError` rather than panicking, so it is still
covered by the no-panic contract.
