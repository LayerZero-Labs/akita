...
fn assert_ntt_matches_coeff<F: FieldCore + CanonicalField + FromSmallInt, const D: usize>() {
    let cfg = test_config();
    let witness = sample_small_instances::<F, D>(NUM_WITNESS_ROWS, WITNESS_LEN, 11);
    let matrix_a = sample_small_instances::<F, D>(cfg.inner_rows, WITNESS_LEN, 12);

    let decomp_len = NUM_WITNESS_ROWS * cfg.inner_rows * cfg.num_digits;
    let matrix_b = sample_small_instances::<F, D>(cfg.outer_rows, decomp_len, 13);
