...
/// Polynomials are split into multiple commitment groups according to
...
fn run_grouped_onehot(nv: usize, batch_size: usize) {
    ...
    run_on_large_stack(move || {
        ...
        // Build group slices according to the partition.
        let mut poly_groups: Vec<&[OneHotPoly<F, D, u8>]> = Vec::new();
        ...
        // Cross-check: commit each group individually and compare.
        for (group_idx, &gs) in group_sizes.iter().enumerate() {
            let single_group: [&[OneHotPoly<F, D, u8>]; 1] =
                [&polys[group_sizes[..group_idx].iter().sum::<usize>()
                    ..group_sizes[..group_idx].iter().sum::<usize>() + gs]];
            let (individual_commits, _) = <HachiCommitmentScheme<D, Cfg> as CommitmentScheme<
                F,
                D,
            >>::batched_commit(
                &single_group, &setup, &layout
            )
