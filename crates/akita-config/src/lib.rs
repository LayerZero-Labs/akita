...
use akita_planner::{
    fold_level_params_from_entry, generated::table_entry, validate_catalog_identity, PlannerPolicy,
};
use akita_transcript::{append_ext_field, sample_ext_challenge, Transcript};
#[cfg(test)]
use akita_types::PolynomialGroupLayout;
use akita_types::{
    AkitaExpandedSetup, AkitaScheduleInputs, AkitaScheduleLookupKey, ChunkedWitnessCfg,
    DecompositionParams, FoldAOnesTable, LevelParams, OpeningClaimsLayout, Schedule,
        ...
        Self::Field: FromPrimitiveInt,
    {
        ...
        for key in keys {
            ...
            if let Some(ref catalog_table) = catalog {
                if let Some(entry) = table_entry(*catalog_table, &key) {
            ...
            if !warmed {
                let layout = OpeningClaimsLayout::new(
                    key.final_group.num_vars(),
                    key.final_group.num_polynomials(),
                )?;
                if let Ok(schedule) = Self::get_params_for_prove(&layout) {
