...
pub trait CommitmentConfig: Clone + Send + Sync + 'static {
    ...
    /// Resolve the unique catalog-authorized profile for a group committed
    /// before a later multi-group opening.
    ///
    /// Unlike [`Self::profile_without_precommitted_groups`], this does not
    /// require the group to have a complete standalone opening schedule.
