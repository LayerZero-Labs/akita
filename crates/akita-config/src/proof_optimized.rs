...
/// Response-tail headroom for states with an exact measured source energy.
///
/// The largest observed response-to-conditional-mean ratio across the three
/// field profiles was 1.055525. A 1.25 multiplier leaves more than 18 percent
/// residual margin over that observation while preserving the useful SIS
...
const FP128_ONEHOT_L2_CAPS: &[akita_schedules::SelectiveL2FoldCap] = &[
    ...
    // length also opts this family into the general balanced-digit model.
    akita_schedules::SelectiveL2FoldCap::from_source_energy_model(
        3,
        511_872,
        5,
    ...
    // Calibrated on the shipped (40, 1) source state. The rejection-sampled
    // challenge has ||rho||_2^2 = 75, so the exact conditional mean is
    // 75 * 9,441,218 = 708,091,350. The shared response-tail headroom remains
    // below the D64 collision bucket boundary where rank three supports the
    // canonical split. Complete-suffix pricing decides whether it wins.
    akita_schedules::SelectiveL2FoldCap::from_source_energy_model(
        3,
        2_083_904,
        3,
...
const FP64_D64_ONEHOT_L2_CAPS: &[akita_schedules::SelectiveL2FoldCap] = &[
    akita_schedules::SelectiveL2FoldCap::from_source_energy_model(
        3,
        594_624,
        3,
...
macro_rules! impl_proof_optimized_preset {
    ...
    ($cfg:ident, $field:ty, $ext_field:ty, $family:expr, $d:expr, $field_bits:expr, $log_commit_bound:expr, fold_norms = $fold_norms:expr, schedules = ($feat:literal, $family_name:literal, $table:ident), ring_dimension_schedule_mode = $mode:expr, selective_l2_caps = $selective_l2_caps:expr) => {
        impl_proof_optimized_preset!(@core $cfg, $field, $ext_field, $family, $d, $field_bits, $log_commit_bound, $fold_norms, $selective_l2_caps, table, $feat, $family_name, $table, ring_dimension_schedule_mode = $mode);
    };
    (@options default) => {
