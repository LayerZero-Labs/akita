...
/// `Self`-generic proof-optimized `schedule_plan` entry. Materializes `Cfg`'s
/// generated table under `Cfg`'s own policy (including
/// `Cfg::terminal_proof_mode()`), so a wrapper that overrides only the terminal
/// mode reuses the table's root/fold structure while the terminal step is sized
/// for the selected mode. Presets and the test-only `DirectTerminalCfg` route
/// `schedule_plan` through this.
pub fn schedule_plan_for<Cfg: CommitmentConfig>(
    key: AkitaScheduleLookupKey,
) -> Result<Option<AkitaSchedulePlan>, AkitaError> {
    proof_optimized_schedule_plan::<Cfg>(key, Cfg::envelope(key.num_vars))
}
    ...
    ($cfg:ident, $d:expr, $log_commit_bound:expr, $table:expr) => {
        impl $crate::CommitmentConfig for $cfg {
            ...
            fn schedule_plan(
                ...
                $crate::proof_optimized::schedule_plan_for::<Self>(key)
            }
