...
use akita_types::{AkitaScheduleLookupKey, Schedule};
...
fn check_family(family: &GeneratedFamily, into: &mut Vec<Mismatch>) {
    ...
    for key in keys {
        ...
        let table_str = render_schedule(&table_backed);
        let regen_str = render_schedule(&regenerated);
        if table_str != regen_str {
            into.push(Mismatch {
                family: family.module_name,
                key,
                table_backed: table_str,
                regenerated: regen_str,
            });
        }
    }
}
