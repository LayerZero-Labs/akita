...
use crate::ring::{CrtNttParamSet, CyclotomicCrtNtt, CyclotomicRing};
...
fn row_ring_slice<F: FieldCore, const D: usize>(
    data: &[F],
    num_cols: usize,
    row: usize,
) -> &[CyclotomicRing<F, D>] {
    let row_field_len = num_cols * D;
    let start = row * row_field_len;
    let field_slice = &data[start..start + row_field_len];
    ...
    W: PrimeWidth,
{
    ...
    cfg_into_iter!(0..total)
        .map(|idx| {
            let r = idx / num_cols;
            let c = idx % num_cols;
            CyclotomicCrtNtt::from_ring_with_params(&row_ring_slice(data, num_cols, r)[c], params)
        })
        .collect()
}
    ...
    W: PrimeWidth,
{
    ...
    cfg_into_iter!(0..total)
        .map(|idx| {
            let r = idx / num_cols;
            let c = idx % num_cols;
            CyclotomicCrtNtt::from_ring_cyclic(&row_ring_slice(data, num_cols, r)[c], params)
        })
        .collect()
}
...
fn build_ntt_slot_from_params<F: FieldCore + CanonicalField, const D: usize>(
    ...
    match params {
        ProtocolCrtNttParams::Q32(p) => {
            let (neg, cyc) = cfg_join!(|| convert_flat(data, num_rows, num_cols, &p), || {
                convert_flat_cyclic(data, num_rows, num_cols, &p)
            });
            NttSlotCache::Q32 {
                neg,
                cyc,
