...
pub fn partial_evaluations<H: SwitchField>(
    ...
    equality_scratch[0] = H::ONE;
    for (axis, &r) in point.iter().enumerate() {
        let width = 1 << axis;
        for j in 0..width {
            let hi = equality_scratch[j] * r;
            equality_scratch[j + width] = hi;
            equality_scratch[j] += hi;
        }
    }
...
/// a small nibble lookup avoids one F162 operation per individual host bit.
pub fn batched_weights<H: SwitchField>(
    ...
    output.resize(host_weights.len(), F::ZERO);
    for (out, &host) in output.iter_mut().zip(host_weights) {
        *out = F::ZERO;
        for (word_index, word) in host.coordinates()[..H::ROWS / 64].iter().enumerate() {
            for nibble in 0..16 {
                *out += lookup[word_index * 16 + nibble][((word >> (4 * nibble)) & 15) as usize];
            }
        }
    }
    Ok(())
