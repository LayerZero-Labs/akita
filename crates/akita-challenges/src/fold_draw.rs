...
///
/// The prefix binds the group index, exact `num_blocks`, claim count, and
...
pub fn fold_challenge_sample_label(
    ...
    let num_blocks = u64::try_from(num_blocks)
        .map_err(|_| AkitaError::InvalidSetup("num_blocks exceeds u64".to_string()))?;
    ...
    label.extend_from_slice(&num_blocks.to_le_bytes());
...
pub trait FoldDraw {
    ...
    fn draw_folding_challenges(
        ...
        if num_blocks == 0 || num_claims == 0 {
            return Err(AkitaError::InvalidInput(
                "fold challenges require positive num_blocks and claims".to_string(),
            ));
        }
            ...
            ChallengeShape::Tensor { fold_low_len } => {
                ...
                Challenges::from_tensor_dyn(
                    TensorChallenges {
                        fold_high,
                        fold_low,
                        live_blocks_per_claim: num_blocks,
