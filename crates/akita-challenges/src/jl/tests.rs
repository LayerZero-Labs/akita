...
fn fast_kernel_matches_reference_kernel() {
    let n_rows = 32;
    let cols: usize = 1023; // non-multiple of 8
    let signs: Vec<Vec<i8>> = (0..n_rows)
...
fn parallel_column_panels_match_reference() {
    ...
    // reduces per-panel partials. `cols` is not a multiple of 8 to also cover
...
fn explicit_zero_sign_rows_are_rejected() {
    ...
    assert!(JlProjectionMatrix::from_sign_rows(&signs).is_err());
}
...
fn check_l2_accepts_generous_and_rejects_tight() {
    let signs = vec![vec![1i8, 1i8, 1i8], vec![1i8, 1i8, -1i8]];
    let coeffs = [F64::from_i64(3), F64::from_i64(-4), F64::from_i64(5)];
    let matrix = JlProjectionMatrix::from_sign_rows(&signs).unwrap();
    let image = matrix.project(&coeffs).unwrap();
    assert_eq!(image.coords(), &[4, -6]);

    assert_eq!(image.l2_norm_sq_checked().unwrap(), 52);
    assert!(image.check_l2(52).is_ok());
    assert!(image.check_l2(51).is_err());
}
