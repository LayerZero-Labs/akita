...
/// Gather the sixteen ternary signs for four packed row bytes into one vector.
#[inline]
unsafe fn signs_for_4bytes(row: &[u8], byte_base: usize) -> int8x16_t {
    let mut signs = [0i8; 16];
    signs[0..4].copy_from_slice(&SIGNS_FOR_BYTE[row[byte_base] as usize]);
    signs[4..8].copy_from_slice(&SIGNS_FOR_BYTE[row[byte_base + 1] as usize]);
    signs[8..12].copy_from_slice(&SIGNS_FOR_BYTE[row[byte_base + 2] as usize]);
    signs[12..16].copy_from_slice(&SIGNS_FOR_BYTE[row[byte_base + 3] as usize]);
...
pub(super) unsafe fn project_row_neon(row: &[u8], digits: &[i8], cols: usize) -> i32 {
    ...
    for chunk in 0..chunk4 {
        let signs = signs_for_4bytes(row, chunk << 2);
