...
impl IntegerChallenge {
        ...
        E: FieldCore + MulBase<F>,
    {
        ...
        for (&pos, &coeff) in self.positions.iter().zip(self.coeffs.iter()) {
            ...
            if idx >= ring_d {
                return Err(AkitaError::InvalidInput(format!(
                    "integer challenge position {idx} out of range for D={ring_d}"
                )));
            }
            if coeff == 0 {
                return Err(AkitaError::InvalidInput(
...
mod tests {
    ...
    fn integer_challenge_eval_matches_sparse_when_in_range() {
        ...
        let pows = alpha_pows();
        let sparse_eval = sparse.eval_at_pows::<F, F>(&pows).unwrap();
        let widened_eval = widened.eval_at_pows::<F, F>(&pows).unwrap();
