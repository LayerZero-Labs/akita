...
mod tests {
    ...
    use akita_types::{
        relation_claim_from_fold_active_rows_for_level, ring_opening_point_from_field,
        AkitaCommitmentHint, BasisMode, BlockOrder, FoldAOnesTable, FoldStep, LevelParams,
    ...
    fn ring_multiplier_root_rows_match_direct_relation_claim() {
        ...
        for row in 0..rows {
            ...
            let y = instance.y();
            let b_start = lp.b_start().expect("b_start");
            let expected = relation_claim_from_fold_active_rows_for_level::<F, D>(
                &lp,
                MRowLayout::WithDBlock,
                1,
                &tau1,
