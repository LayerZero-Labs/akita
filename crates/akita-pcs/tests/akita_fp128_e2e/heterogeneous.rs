...
fn heterogeneous_group_types() {
    ...
    run_on_large_stack(|| {
        ...
        let stack = CpuBackend::new::<OneHotCfg>(setup.expanded.clone(), onehot_scheme.schedules())
            .expect("backend");
        ...
        let akita_cpu_backend::CommitOutput {
            ...
            .commit::<OneHotCfg>(
                &stack
                    .import_source::<OneHotCfg, _>(vec![onehot_pre.clone()])
                    .expect("source"),
        ...
        let dense_setup = dense_scheme
            ...
            .expect("dense setup");
        let dense_stack =
            CpuBackend::new::<DenseCfg>(dense_setup.expanded.clone(), dense_scheme.schedules())
                .unwrap();
        let akita_cpu_backend::CommitOutput {
            ...
            .commit::<DenseCfg>(
                &dense_stack
                    .import_source::<DenseCfg, _>(dense_polys.to_vec())
                    .expect("source"),
                akita_cpu_backend::GroupContext::scheduler_without_precommitted_groups(),
            )
...
fn bounded_dense_precommit_with_onehot_final_group() {
    ...
    run_on_large_stack(|| {
        ...
        let bounded_stack = CpuBackend::new::<BoundedDenseCfg>(
        ...
        let akita_cpu_backend::CommitOutput {
            ...
            .commit::<BoundedDenseCfg>(
                &bounded_stack
                    .import_source::<BoundedDenseCfg, _>(vec![bounded_dense.clone()])
                    .expect("source"),
                akita_cpu_backend::GroupContext::scheduler_without_precommitted_groups(),
            )
...
fn commit_rejects_a_source_whose_representation_is_not_the_declared_class() {
    ...
    run_on_large_stack(|| {
        ...
        let error = stack
            .commit::<OneHotCfg>(
                &stack
                    .import_source::<OneHotCfg, _>(vec![dense.clone()])
                    .expect("source"),
                akita_cpu_backend::GroupContext::scheduler_without_precommitted_groups(),
            )
            .map(|_| ())
        ...
        stack
            .commit::<OneHotCfg>(
                &stack
                    .import_source::<OneHotCfg, _>(vec![onehot.clone()])
                    .expect("source"),
                akita_cpu_backend::GroupContext::scheduler_without_precommitted_groups(),
            )
...
fn bounded_dense_commit_rejects_a_coefficient_above_the_declared_bound() {
    ...
    run_on_large_stack(|| {
        ...
        let stack =
            CpuBackend::new::<BoundedDenseCfg>(setup.expanded.clone(), bounded_scheme.schedules())
                .expect("backend");
        ...
        let commit = |evals: &[F]| {
            ...
            stack
                .commit::<BoundedDenseCfg>(
                    &stack
                        .import_source::<BoundedDenseCfg, _>(vec![poly.clone()])
                        .expect("source"),
                    akita_cpu_backend::GroupContext::scheduler_without_precommitted_groups(),
                )
                .map(|_| ())
        ...
        let full_stack =
            CpuBackend::new::<DenseCfg>(full_setup.expanded.clone(), dense_scheme.schedules())
                .unwrap();
        let poly =
        ...
        full_stack
            .commit::<DenseCfg>(
                &full_stack
                    .import_source::<DenseCfg, _>(vec![poly.clone()])
                    .expect("source"),
                akita_cpu_backend::GroupContext::scheduler_without_precommitted_groups(),
            )
...
fn explicit_commitment_transfer_between_backends() {
    ...
    run_on_large_stack(|| {
        ...
        let receiver = CpuBackend::new::<Cfg>(setup.expanded.clone(), scheme.schedules()).unwrap();
        let hint = receiver
