...
use akita_types::{ClaimIncidenceSummary, FlatDigitBlocks};
...
fn custom_commit_source_runs_commit_with_params() {
    ...
    let incidence = ClaimIncidenceSummary::same_point(NUM_VARS, 1).expect("incidence");
    let params = Cfg::get_params_for_batched_commitment(&incidence).expect("layout");

    let setup_envelope = Cfg::max_setup_matrix_size(NUM_VARS, 1, 1).expect("envelope");
    let setup = AkitaProverSetup::<F, D>::generate_with_capacity(NUM_VARS, 1, 1, setup_envelope)
        .expect("setup");
