...
use akita_types::{
    ...
    AkitaScheduleLookupKey, CommitmentRingDims, CommittedGroupProfile, FoldSchedule,
...
type W8R2PrecommittedCfg = PrecommittedCommitmentConfig<fp128::OneHotMultiChunk>;
...
fn w8r2_verifier_setup_stops_after_the_offloaded_chain() {
    ...
    assert_eq!(
        &incoming_prefixes[..2],
        &[Some(12_681_216), Some(7_840_768)]
    );
    assert!(incoming_prefixes[2..].iter().all(Option::is_none));
    assert_eq!(prover.num_field_elements, 12_681_216);
    assert_eq!(verifier.num_field_elements, 5_046_272);
    assert_eq!(setup_for_two.num_field_elements, 8_388_608);
    assert_eq!(setup_for_four.num_field_elements, 12_681_216);
}
...
/// would detect. The single-chunk profile also fails this on levels 0/1.
