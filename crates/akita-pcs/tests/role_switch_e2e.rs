//! Multi-level per-role "role-switch" E2E acceptance test.
//!
//! - L0 (root): `d_a/d_b/d_d = 128/128/64` (compress D only).
//! - L1: `128/64/64` (compress B and D).
//! - L2+: uniform `64`.
//!
//! Exercises per-role commitment compression at BOTH the root and a recursive
//! fold, through the public PCS API, with tamper rejection.
...
const LABEL: &[u8] = b"test/role_switch_e2e";
...
#[test]
fn role_switch_proves_verifies_and_rejects_tamper() {
    init_rayon_pool();
    run_on_large_stack(|| {
        ...
        .expect("role-switch prove");

        verify_with(&verifier_setup, &proof, &point, &[opening], &commitment)
            .expect("honest role-switch proof must verify");
