...
// returns `Tensor` at the root level. The planner consults the selector through
// `LevelParams::challenge_l1_mass`, sizes `num_digits_fold` for the wider
// tensor envelope, and bakes `TensorChallengeShape::Tensor` into the root fold
// step's `params.fold_challenge_shape`.
// ---------------------------------------------------------------------------

#[path = "single_poly_e2e/planner_tensor_fold.rs"]
