...
///
/// Important: onehot polys bake their `(fold_bits, position_bits)` block split in at
/// construction time, unlike dense polys which rebuild blocks from the
/// prover-supplied `fold_position_count`. Under batched commits that split must match
