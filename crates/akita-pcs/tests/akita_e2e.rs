...
mod non_zk_static_and_direct {
    ...
    fn fp32_static_dense_round_trip() {
        ...
        run_on_large_stack(|| {
            ...
            let mut verifier_transcript =
                Blake2bTranscript::<FSmall>::new(b"akita_e2e/fp32-static");
            let result =
    ...
    fn fp64_static_dense_round_trip() {
        ...
        run_on_large_stack(|| {
            ...
            let (verifier_setup, commitment, proof, opening_point, opening, _layout) =
                make_dense_fixture::<FSmall, D, Cfg>(
                    SMALL_FIELD_TEST_NV + 1,
                    b"akita_e2e/fp64-static",
                );

            let commitments = [commitment];
            let openings = [opening];
            let mut verifier_transcript =
                Blake2bTranscript::<FSmall>::new(b"akita_e2e/fp64-static");
            let result =
                <AkitaCommitmentScheme<D, Cfg> as CommitmentVerifier<FSmall, D>>::batched_verify(
                    &proof,
                    &verifier_setup,
...
mod non_zk_onehot_schedule {
    ...
    fn adaptive_onehot_direct_tail_uses_terminal_schedule_basis() {
        ...
        run_on_large_stack(|| {
            ...
            let mut prover_transcript =
                Blake2bTranscript::<F>::new(b"akita_e2e/onehot-direct-tail");
            let proof = <AkitaCommitmentScheme<D, Cfg> as CommitmentProver<F, D>>::batched_prove(
                &setup,
                prove_input(
                    &pt[..],
...
mod non_zk_schedule_envelope {
    ...
    fn batched_onehot_4x30_keeps_folding_past_oversized_tail() {
        ...
        run_on_large_stack(|| {
            ...
            let mut cursor = std::io::Cursor::new(serialized);
            let decoded =
                AkitaBatchedProof::<F, F>::deserialize_compressed(&mut cursor, &proof_shape)
                    .expect("deserialize batched onehot proof");
