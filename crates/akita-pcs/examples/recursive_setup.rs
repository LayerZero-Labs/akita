...
fn run_mode(
    ...
    let setup = match mode {
        SetupContributionMode::Direct => <Scheme as CommitmentProver<F, D>>::setup_prover(nv, 1, 1),
        SetupContributionMode::Recursive => {
            <Scheme as CommitmentProver<F, D>>::setup_prover_recursion(nv, 1, 1)
        }
    }
    .expect("setup_prover");
...
fn main() {
    ...
    let nv = env_usize("AKITA_NUM_VARS", 32);
    ...
    assert_eq!(
        total_ring * ONEHOT_K,
        1usize << nv,
        "AKITA_NUM_VARS={nv} must match the D=32 OneHot layout"
    );
