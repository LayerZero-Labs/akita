        ...
        RootCommitBackend<FF, P, Cfg::ExtField, D> + RecursiveProveBackend<FF, P, Cfg::ExtField, D>,
{
    ...
    // TEMP BENCH HOOK: repeat verify `AKITA_VERIFY_REPEAT` times (default 1),
    // snapshotting the per-span busy-time aggregator each iteration.
    let verify_repeat = env_usize("AKITA_VERIFY_REPEAT", 1).max(1);
    let mut agg_runs: Vec<Vec<(String, std::time::Duration, u64)>> = Vec::new();
    for iter in 0..verify_repeat {
        crate::span_agg::reset();
        let t0 = Instant::now();
        ...
        match <Scheme<D, Cfg> as CommitmentVerifier<FF, D>>::batched_verify(
            ...
            Ok(()) => {
                ...
                report_timing(label, &format!("verify OK [iter {iter}]"), secs);
                let snap = crate::span_agg::snapshot();
                crate::report::report_span_breakdown(label, iter, secs, &snap);
                agg_runs.push(snap);
            }
            Err(e) => {
    ...
    crate::report::report_span_breakdown_avg(label, &agg_runs);
}
