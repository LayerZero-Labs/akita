...
/// TEMP BENCH HOOK: print one verify iteration's per-span busy-time breakdown.
pub(crate) fn report_span_breakdown(
    label: &str,
    iter: usize,
    total_secs: f64,
    snap: &[(String, std::time::Duration, u64)],
) {
    eprintln!(
        "[{label}] === verify span breakdown [iter {iter}] (total verify {:.6}s) ===",
        total_secs
    );
    for (name, dur, count) in snap {
        if name.starts_with("L") && name.contains('/') {
            continue;
        }
        eprintln!(
            "[{label}]   {:<34} {:>12.3} ms  (n={count})",
            name,
            dur.as_secs_f64() * 1e3,
        );
    }
...
/// TEMP BENCH HOOK: print the per-span average across all verify iterations.
pub(crate) fn report_span_breakdown_avg(
    label: &str,
    runs: &[Vec<(String, std::time::Duration, u64)>],
) {
    if runs.is_empty() {
        return;
    }
    ...
    for run in runs {
        for (name, dur, count) in run {
            let entry = totals.entry(name.clone()).or_insert((0.0, 0));
            entry.0 += dur.as_secs_f64() * 1e3;
            entry.1 += count;
        }
    }
    let n = runs.len() as f64;
    let mut rows: Vec<(String, f64, f64)> = totals
        .into_iter()
        .map(|(name, (ms, cnt))| (name, ms / n, cnt as f64 / n))
        .collect();
    rows.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
    eprintln!(
        "[{label}] === verify span breakdown AVERAGE over {} iters ===",
        runs.len()
    );
    for (name, ms, cnt) in &rows {
        ...
        eprintln!("[{label}]   {name:<34} {ms:>12.3} ms  (avg n={cnt:.0})");
    }
    report_span_breakdown_by_level(label, &rows);
}

/// TEMP BENCH HOOK: per-fold-level average for Matrix-M row-eval components.
pub(crate) fn report_span_breakdown_by_level(label: &str, rows: &[(String, f64, f64)]) {
    use std::collections::BTreeMap;
    ...
    let mut by_level: BTreeMap<usize, BTreeMap<&str, f64>> = BTreeMap::new();
    for (name, ms, _cnt) in rows {
        let Some((level, component)) = name.split_once('/') else {
            continue;
        };
        let Some(level_idx) = level.strip_prefix('L').and_then(|s| s.parse().ok()) else {
            continue;
        };
        if COMPONENTS.contains(&component) {
            by_level.entry(level_idx).or_default().insert(component, *ms);
        }
    }
    if by_level.is_empty() {
        return;
    }
    eprintln!("[{label}] === per-level row-eval breakdown (ms, avg over iters) ===");
    eprintln!(
        "[{label}]   {:>4}  {:>10}  {:>10}  {:>10}  {:>10}  {:>10}  {:>10}",
        "L", "setup", "z", "e", "t", "r_dense", "row_total"
    );
    for level_idx in by_level.keys() {
        let cols = &by_level[level_idx];
        let setup = cols.get("setup_contribution").copied().unwrap_or(0.0);
        let z = cols.get("z_structured").copied().unwrap_or(0.0);
        let e = cols.get("e_structured").copied().unwrap_or(0.0);
        let t = cols.get("t_structured").copied().unwrap_or(0.0);
        let r = cols.get("r_dense").copied().unwrap_or(0.0)
            + cols.get("r_structured").copied().unwrap_or(0.0);
        let total = setup + z + e + t + r;
        eprintln!(
            "[{label}]   L{level_idx:<3}  {setup:10.3}  {z:10.3}  {e:10.3}  {t:10.3}  {r:10.3}  {total:10.3}"
        );
    }
    let sum = |key: &str| {
        by_level
            .values()
            .map(|cols| cols.get(key).copied().unwrap_or(0.0))
            .sum::<f64>()
    };
    let setup = sum("setup_contribution");
    let z = sum("z_structured");
    let e = sum("e_structured");
    let t = sum("t_structured");
    let r = sum("r_dense") + sum("r_structured");
    eprintln!(
        "[{label}]   {:>4}  {setup:10.3}  {z:10.3}  {e:10.3}  {t:10.3}  {r:10.3}  {:10.3}",
        "sum", setup + z + e + t + r
    );
}
