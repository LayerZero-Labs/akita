//! Microbenchmarks for sparse stage-1 challenge sampling at `D=32`.
//!
//! These benchmarks compare the previous fp128 `D=32` family
//! `Uniform { weight: 32, nonzero_coeffs: ±[1..8] }` against the new preset
//! `BoundedL1Ball { max_abs_coeff: 8, l1_bound: 121 }` from
//! `specs/bounded-l1-sparse-challenge.md`.
//!
//! For each family we measure:
//!
//! - `singleton`: one `sparse_challenge_from_transcript` call. Includes
//!   transcript absorb, XOF seeding, and (for `BoundedL1Ball`) the WAYS-table
//!   build. Useful for attributing per-call overhead.
//! - `batch_<N>`: one `sample_sparse_challenges(N)` call. Includes one
//!   transcript absorb, one XOF seeding, one WAYS-table build, and `N`
//!   per-challenge decodes. Use this to read the steady-state per-challenge
//!   cost: it amortizes the table build across the whole batch, so the gap
//!   between `Uniform` and `BoundedL1Ball` here reflects the true streaming
//!   DP-decode cost.
//!
//! Run with:
//!
//! ```text
//! cargo bench -p akita-pcs --bench sparse_challenge
//! ```

#![allow(missing_docs)]

use akita_algebra::ring::SparseChallengeConfig;
use akita_challenges::sparse::{sample_sparse_challenges, sparse_challenge_from_transcript};
use akita_config::proof_optimized::fp128;
use akita_field::FromSmallInt;
use akita_transcript::labels::DOMAIN_AKITA_PROTOCOL;
use akita_transcript::{Blake2bTranscript, Transcript};
use criterion::{black_box, criterion_group, criterion_main, BenchmarkId, Criterion, Throughput};

// Use the same 128-bit base field as `crates/akita-pcs/benches/akita_e2e.rs`
// (fp128 = `Prime128OffsetA7F7` per `proof_optimized::fp128`) so that this
// microbench measures the sampler under the same field arithmetic as the
// production stage-1 path.
type F = fp128::Field;

const D: usize = 32;

/// Batch counts that bracket the realistic stage-1 fan-out for `D=32`
/// presets. Smaller counts emphasize the per-call (table build, transcript
/// absorb) overhead; larger counts emphasize the amortized per-challenge
/// decode cost.
const BATCH_SIZES: &[usize] = &[1, 8, 64, 256, 1024, 4096];

fn fresh_transcript() -> Blake2bTranscript<F> {
    let mut t = Blake2bTranscript::<F>::new(DOMAIN_AKITA_PROTOCOL);
    // Mirror what the profile harness does: seed once with a non-empty
    // transcript so we are not measuring the empty-transcript fast path.
    t.append_field(b"bench-seed", &F::from_u64(0xC0FFEE));
    t
}

fn cfg_uniform_d32_legacy() -> SparseChallengeConfig {
    SparseChallengeConfig::Uniform {
        weight: 32,
        nonzero_coeffs: (-8..=8).filter(|&c| c != 0).collect(),
    }
}

fn cfg_bounded_l1_d32() -> SparseChallengeConfig {
    SparseChallengeConfig::BoundedL1Ball {
        max_abs_coeff: 8,
        l1_bound: 121,
    }
}

fn bench_singleton(c: &mut Criterion) {
    let mut group = c.benchmark_group("sparse_challenge_d32_singleton");
    let cases: &[(&str, SparseChallengeConfig)] = &[
        ("uniform_w32_alpha8", cfg_uniform_d32_legacy()),
        ("bounded_l1_m8_b121", cfg_bounded_l1_d32()),
    ];
    for (name, cfg) in cases {
        group.bench_function(BenchmarkId::from_parameter(name), |b| {
            b.iter(|| {
                // Fresh transcript per iteration so the absorb/squeeze cost is
                // realistic for the "one call site, one challenge" case.
                let mut tr = fresh_transcript();
                let challenge = sparse_challenge_from_transcript::<F, _, D>(
                    &mut tr,
                    b"bench/single",
                    0,
                    black_box(cfg),
                )
                .expect("singleton sparse challenge");
                black_box(challenge)
            });
        });
    }
    group.finish();
}

fn bench_batch(c: &mut Criterion) {
    let mut group = c.benchmark_group("sparse_challenge_d32_batch");
    let cases: &[(&str, SparseChallengeConfig)] = &[
        ("uniform_w32_alpha8", cfg_uniform_d32_legacy()),
        ("bounded_l1_m8_b121", cfg_bounded_l1_d32()),
    ];
    for &n in BATCH_SIZES {
        group.throughput(Throughput::Elements(n as u64));
        for (name, cfg) in cases {
            let id = BenchmarkId::new(*name, n);
            group.bench_with_input(id, &n, |b, &n| {
                b.iter(|| {
                    let mut tr = fresh_transcript();
                    let challenges = sample_sparse_challenges::<F, _, D>(
                        &mut tr,
                        b"bench/batch",
                        n,
                        black_box(cfg),
                    )
                    .expect("batch sparse challenges");
                    black_box(challenges)
                });
            });
        }
    }
    group.finish();
}

criterion_group!(sparse_challenge, bench_singleton, bench_batch);
criterion_main!(sparse_challenge);
