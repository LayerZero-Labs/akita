...
fn make_case(live_block_count: usize, blocks_per_chunk: usize) -> SetupIndexWeightBenchCase {
    assert!(live_block_count.is_power_of_two());
    assert!(blocks_per_chunk.is_power_of_two());
    assert!(blocks_per_chunk <= live_block_count);
    assert_eq!(live_block_count % blocks_per_chunk, 0);
    ...
    .with_decomp(
        positions_per_block,
        live_block_count * positions_per_block,
        depth_commit,
        depth_open,
    )
    .unwrap();
    level_params.blocks_per_chunk_granule = 1;
    ...
    let z_range = positions_per_block * depth_commit;
...
fn bench_setup_index_weight(c: &mut Criterion) {
    ...
    for live_block_count in [64usize, 256, 1024, 4096, 16384] {
        for (layout, blocks_per_chunk) in [
            ("single_chunk", live_block_count),
            ("chunk64", 64usize.min(live_block_count)),
        ] {
            let case = make_case(live_block_count, blocks_per_chunk);
            group.bench_with_input(
                BenchmarkId::new(format!("{layout}/packed_path"), live_block_count),
                &case,
                |b, case| {
                    b.iter(|| {
                        black_box(
            ...
            group.bench_with_input(
                BenchmarkId::new(format!("{layout}/succinct_path"), live_block_count),
                &case,
                |b, case| {
