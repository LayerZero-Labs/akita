...
impl<F: FieldCore, const D: usize> AkitaCommitmentHint<F, D> {
    ...
    /// consumed by [`AkitaCommitmentHint::into_flat_parts`]
    /// afterwards. Used by the prover pipeline to thread
    /// `ûhat_concat` into the M-witness while still calling the
    /// legacy `into_flat_parts` to flatten the inner rows.
    #[inline]
