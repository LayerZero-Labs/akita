...
pub struct AkitaExpandedSetup<F: FieldCore> {
    ...
    ///
    /// Tiered roots additionally need a per-setup `F` matrix
    /// (`specs/tiered_commit.md` §3) that is deterministically derived
    /// from `seed.public_matrix_seed` under the `b"tier1-f"`
    /// domain-separation label. Holds `(num_rings, FlatMatrix)` so a
    /// debug-only mismatch check can detect setups being reused at
    /// unexpected tiered shapes. For legacy roots the cache stays
    /// empty forever; clones share the cache via the `Arc` so amortise
    /// across `verify` calls. Skipped by `PartialEq`/`Eq` and all
    /// serialisation paths.
...
impl<F: FieldCore> PartialEq for AkitaExpandedSetup<F> {
    fn eq(&self, other: &Self) -> bool {
        ...
        // of `seed.public_matrix_seed` which is already part of `seed`).
