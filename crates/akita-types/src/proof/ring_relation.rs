...
pub fn ring_relation_segment_lengths<F: FieldCore + CanonicalField>(
    ...
    let live_block_count = lp.live_block_count;
    if live_block_count == 0 {
        return Err(AkitaError::InvalidSetup(
            "live_block_count must be positive".to_string(),
        ));
    }
    ...
    let z_len = depth_fold
        ...
        .and_then(|len| len.checked_mul(lp.positions_per_block))
...
impl<F: FieldCore + CanonicalField> RingRelationInstance<F> {
    ...
    pub fn new(
        ...
        for g in 0..num_groups {
            ...
            let live_block_count_g = challenges.live_block_count_per_claim();
            if group_opening_points[g].live_block_weights.len() != live_block_count_g {
                return Err(AkitaError::InvalidInput(format!(
                    "ring relation group {g} opening point block count does not match challenges"
                )));
            }
            if group_ring_multiplier_points[g].fold_len() != live_block_count_g {
                return Err(AkitaError::InvalidInput(format!(
...
mod tests {
    ...
    fn opening_point(lp: &LevelParams) -> RingOpeningPoint<F> {
        RingOpeningPoint {
            position_weights: vec![F::zero(); lp.positions_per_block],
            live_block_weights: vec![F::zero(); lp.live_block_count],
        }
    }
    ...
    fn test_challenges(lp: &LevelParams, num_claims: usize) -> Challenges {
        let total = lp.live_block_count * num_claims;
    ...
    fn chunk_test_level_params(block_index_bits: usize) -> LevelParams {
        // live_block_count = 2^block_index_bits, positions_per_block = 2^position_index_bits, single-tier.
        LevelParams::params_only(
            crate::SisModulusProfileId::Q32Offset99,
            D,
            2,
    ...
    fn resolve_single_chunk_matches_legacy_offsets() {
        ...
        assert_eq!(unit.live_block_count(), lp.live_block_count);
    }
    ...
    fn resolve_multi_chunk_offsets_contiguous_and_cover_blocks() {
        ...
        for w in [1usize, 2, 4, 8] {
            let mut lp = chunk_test_level_params(3); // live_block_count = 8
            if w > 1 {
            ...
            let blocks_per_chunk = lp.live_block_count / w;
            ...
            // Block windows tile [0, live_block_count).
            assert_eq!(
    ...
    fn resolve_rejects_bad_chunk_count() {
        ...
        // num_chunks = 16 exceeds live_block_count = 8.
        let mut lp = chunk_test_level_params(3);
        lp.witness_chunk = crate::witness::ChunkedWitnessCfg {
            num_chunks: 16,
            num_activated_levels: 1,
    ...
    fn multi_group_segment_layout_resolves_group_shard_product() {
        ...
        for group_index in [1, 0] {
            ...
            let e_source = (0..num_claims * live_block_count * depth_open)
                .map(|index| marker(100 * group_index + index))
                .collect::<Vec<_>>();
            let t_source = (0..num_claims * live_block_count * n_a * depth_open)
            ...
            for unit in layout.units_for_group(group_index).expect("units") {
                let z_source =
                    (0..params.positions_per_block() * params.num_digits_commit() * depth_fold)
                ...
                for claim in 0..num_claims {
                    for block_idx in unit.global_block_range() {
                        for digit in 0..depth_open {
                            expected_e.push(
                                e_source
                                    [(claim * live_block_count + block_idx) * depth_open + digit],
                            );
                        }
                    }
                }
                ...
                for claim in 0..num_claims {
                    for block_idx in unit.global_block_range() {
                        for a_row in 0..n_a {
                            for digit in 0..depth_open {
                                expected_t.push(
                                    t_source[((claim * live_block_count + block_idx) * n_a
                                        + a_row)
                                        * depth_open
                                        + digit],
                                );
