...
pub fn tail_segment_multiplicities_from_layout_for_params(
    ...
    if d == 0 || d != ring_dimension || params.live_block_count() == 0 {
        return Err(AkitaError::InvalidSetup(
    ...
    let group = layout
        ...
        .ok_or(AkitaError::InvalidProof)?;
    let e_unit = d
        .checked_mul(params.live_block_count())
    ...
    F: FieldCore + CanonicalField + HalvingField + AkitaSerialize,
{
    ...
    for (group_index, group) in groups.iter().enumerate() {
        ...
        let inner_width = group.params.positions_per_block() * depth_commit;
    ...
    F: FieldCore + CanonicalField + HalvingField,
{
    ...
    let inner_width = lp.positions_per_block * depth_commit;
    ...
    let w_block_count = num_w_vectors * lp.live_block_count;
    ...
    let t_block_count = num_t_vectors * lp.live_block_count;
        ...
        for claim in 0..num_claims {
            for global_block in unit.global_block_range() {
                for digit in 0..depth_open {
                    let source =
                        (claim * source_live_block_count + global_block) * depth_open + digit;
                    write_witness_plane(
                        out,
    ...
    for unit in layout.units_for_group(group_id)? {
        for claim in 0..num_claims {
            for global_block in unit.global_block_range() {
                for a_row in 0..n_a {
                    for digit in 0..depth_open {
                        let source = (claim * source_live_block_count + global_block)
                            * planes_per_block
                            + a_row * depth_open
                            + digit;
                        write_witness_plane(
...
pub fn emit_witness_z_planes<const D: usize>(
    ...
    let expected = positions_per_block
        .checked_mul(depth_commit)
        .and_then(|n| n.checked_mul(depth_fold))
