...
/// Commitment-side routing: which committed-polynomial bundle holds each slot.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl CommitmentRouting {
    /// Copy routing from an opening batch.
    pub fn copy_opening_batch(batch: &OpeningBatch) -> Result<Self, AkitaError> {
        let num_claims = batch.num_claims();
        Self {
            claim_to_commitment_group: batch.claim_to_commitment_group().to_vec(),
            claim_poly_in_commitment_group: batch.claim_poly_indices().to_vec(),
    ...
    pub fn from_recursive_opening_batch(num_claims: usize) -> Result<Self, AkitaError> {
        Self {
    ...
    /// Validate that commitment routing is the same as the opening-batch routing.
    pub fn check_matches_opening_batch(&self, batch: &OpeningBatch) -> Result<(), AkitaError> {
        if self.claim_to_commitment_group.as_slice() != batch.claim_to_commitment_group()
            || self.claim_poly_in_commitment_group.as_slice() != batch.claim_poly_indices()
            || self.num_polys_per_commitment_group.as_slice()
                != batch.num_polys_per_commitment_group()
        {
            return Err(AkitaError::InvalidInput(
