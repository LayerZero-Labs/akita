...
fn with_decomp_derives_exact_live_block_geometry() {
    ...
    assert_eq!(lp.live_ring_elements_per_claim, 17);
    assert_eq!(lp.positions_per_block, 8);
    assert_eq!(lp.live_block_count, 3);
    assert_eq!(lp.blocks_per_chunk_granule, 1);
...
fn with_layout_keeps_self_ranks() {
    ...
    assert_eq!(lp.live_block_count, layout_lp.live_block_count);
    assert_eq!(lp.positions_per_block, layout_lp.positions_per_block);
