...
/// Checked address geometry for the flat Stage-2 relation domain.
///
/// The relation binds the largest low coefficient block shared by every
/// relation role and the outgoing witness representation. The remaining
/// Boolean coordinates address relation lanes and padded witness columns.
/// This type is the canonical authority for that split.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct RelationAddressGeometry {
    role_dims: CommitmentRingDims,
    outgoing_ring_dim: usize,
    opening_source_len: usize,
    opening_domain_len: usize,
    coeff_count: usize,
    lane_capacity: usize,
}

impl RelationAddressGeometry {
    /// Validate and construct the flat relation address geometry.
    ///
    /// # Errors
    ///
    ...
    /// padded opening domain overflows.
    pub fn new(
        role_dims: CommitmentRingDims,
        outgoing_ring_dim: usize,
        opening_source_len: usize,
        ...
        let opening_domain_len = opening_domain_len(opening_source_len)?;
        let field_len = opening_domain_len
            .checked_mul(outgoing_ring_dim)
            .ok_or_else(|| AkitaError::InvalidSetup("relation point domain overflow".into()))?;
        let lane_capacity = field_len / coeff_count;
        if !field_len.is_power_of_two() || lane_capacity == 0 || !lane_capacity.is_power_of_two() {
            return Err(AkitaError::InvalidSetup(
                "relation point domain must be a non-zero power of two".into(),
            ));
        }
    ...
    /// Per-role ring dimensions used by this relation.
    #[must_use]
    pub const fn role_dims(self) -> CommitmentRingDims {
        self.role_dims
    }

    /// Ring dimension used to store the outgoing witness.
    #[must_use]
    pub const fn outgoing_ring_dim(self) -> usize {
        self.outgoing_ring_dim
    }

    /// Exact number of outgoing ring elements in the opening source.
    #[must_use]
    pub const fn opening_source_len(self) -> usize {
        self.opening_source_len
    }

    /// Padded number of outgoing ring elements in the Boolean opening domain.
    #[must_use]
    pub const fn opening_domain_len(self) -> usize {
        self.opening_domain_len
    }

    /// Low coefficient block shared by all roles and the outgoing witness.
    #[must_use]
    pub const fn coeff_count(self) -> usize {
        self.coeff_count
    }

    /// Number of Boolean coordinates in the common coefficient block.
    #[must_use]
    pub const fn coeff_variable_count(self) -> usize {
    ...
    /// Number of Boolean coordinates above the common coefficient block.
    #[must_use]
    pub const fn lane_variable_count(self) -> usize {
        self.lane_capacity.trailing_zeros() as usize
    }

    /// Total number of Boolean coordinates in the flat relation domain.
    #[must_use]
    pub const fn point_variable_count(self) -> usize {
        self.coeff_variable_count() + self.lane_variable_count()
    }

    /// Exact opening-source length when expressed in common relation lanes.
    ///
    /// # Errors
    ///
    /// Returns an error if the exact lane count overflows.
    pub fn common_opening_source_len(self) -> Result<usize, AkitaError> {
        self.opening_source_len
            .checked_mul(self.outgoing_ring_dim / self.coeff_count)
            .ok_or_else(|| AkitaError::InvalidSetup("common opening domain overflow".into()))
    }

    /// Number of common coefficient lanes in one ring element for `role`.
    #[must_use]
    pub const fn role_lane_count(self, role: RingRole) -> usize {
        self.role_dims.dim_for(role) / self.coeff_count
    }

    /// Check that a multilinear point addresses this complete padded domain.
    ///
    /// # Errors
    ///
    /// Returns [`AkitaError::InvalidSize`] for a point of the wrong length.
    pub fn validate_point_len(self, actual: usize) -> Result<(), AkitaError> {
        let expected = self.point_variable_count();
        if actual != expected {
