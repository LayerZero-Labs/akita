    ...
    E: ExtField<F> + FromPrimitiveInt,
{
    ...
    debug_assert_eq!(block_rows.len(), layout.live_fold_count * ring_len);
    for (plane, gadget_scalar) in gadget_scalars.iter().enumerate() {
        let gadget = E::lift_base(*gadget_scalar);
        for block in 0..layout.live_fold_count {
    ...
    E: ExtField<F> + FromPrimitiveInt,
{
    ...
    let ring_len = layout.ring_len();
    let mut block_rows = vec![E::zero(); layout.live_fold_count * ring_len];
