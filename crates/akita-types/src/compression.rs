//! Source-independent compressed-commitment plans and compact witnesses.
//!
//! These types deliberately name no protocol role, group, schedule, or claim.
//! They are the checked arithmetic boundary shared by diagnostic planning and
//! execution.
...
/// Maximum complete B/D image accepted by the current compression ladder.
...
/// Maximum number of maps in the current compression ladder.
pub const MAX_COMPRESSION_MAPS: usize = 3;

/// Minimum number of maps in the current compression ladder.
pub const MIN_COMPRESSION_MAPS: usize = 2;
...
impl CompressionChainPlan {
    /// Validate an already selected current-ladder chain.
    pub fn new(
        modulus_profile: SisModulusProfileId,
        source_coefficients: usize,
        maps: Vec<CompressionMapPlan>,
        ...
        if !(MIN_COMPRESSION_MAPS..=MAX_COMPRESSION_MAPS).contains(&maps.len()) {
            return Err(AkitaError::InvalidSetup(format!(
                "compression chain must contain {MIN_COMPRESSION_MAPS}..={MAX_COMPRESSION_MAPS} maps"
            )));
        }
    ...
    /// Select and validate the current complete-image ladder.
...
mod tests {
    ...
    fn ladder_geometry_and_complete_image_bound_are_checked() {
        for (profile, field_bytes, dimensions) in [
            (SisModulusProfileId::Q128OffsetA7F7, 16, vec![32, 16, 8]),
            (SisModulusProfileId::Q64Offset59, 8, vec![64, 32, 16]),
            (SisModulusProfileId::Q32Offset99, 4, vec![128, 64, 32]),
        ] {
