...
pub fn setup_required_for_inputs<E: FieldCore>(
    ...
    if inputs.live_block_count == 0 {
        return Err(AkitaError::InvalidSetup(
            "live_block_count must be positive".into(),
        ));
    }
    if inputs.positions_per_block == 0
        || inputs.depth_open == 0
...
mod tests {
    ...
    fn single_chunk_layout_from_inputs(inputs: &SetupContributionPlanInputs<F>) -> WitnessLayout {
        let z_len = inputs.positions_per_block * inputs.depth_commit * inputs.depth_fold;
        let e_len = inputs.num_claims * inputs.live_block_count * inputs.depth_open;
