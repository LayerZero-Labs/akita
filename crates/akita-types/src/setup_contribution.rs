...
impl<E: FieldCore> SetupContributionPlan<E> {
        ...
        E: MulBase<F>,
    {
        ...
        if num_chunks > 1 && (inputs.num_segments != 1 || inputs.tier_split > 1) {
            return Err(AkitaError::InvalidSetup(
                "multi-chunk setup contribution requires a single non-tiered bundle".into(),
            ));
        }
        ...
        // Tiered `û_concat` offset lives on the (single) last chunk.
        let (last_chunk, _) = chunk_layout.last_chunk()?;
        let offset_u = last_chunk.offset_u.unwrap_or(0);
