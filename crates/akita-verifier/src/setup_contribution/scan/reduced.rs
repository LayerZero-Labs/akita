...
pub(super) fn evaluate_groups_reduced<F, E, const BASE_D: usize>(
...
{
    let (functional_classes, scan_groups) = reduced_functional_classes::<_, BASE_D>(plan, weights)?;
    if E::SUM_IS_EXACT {
        evaluate_groups_reduced_with::<F, _, BASE_D, DelayedProductSum<E>>(
            plan,
            setup_view,
            &functional_classes,
            &scan_groups,
        ...
        evaluate_groups_reduced_with::<F, _, BASE_D, CanonicalProductSum<E>>(
            plan,
            setup_view,
            &functional_classes,
            &scan_groups,
...
fn reduced_functional_classes<'a, E: Field, const BASE_D: usize>(
    plan: &SetupContributionPlan<E>,
    weights: &'a [ReducedDirectScanWeights<E>],
) -> Result<ReducedFunctionalClasses<'a, E>, AkitaError> {
    if plan.groups().len() != weights.len() {
            ...
            let class = if let Some(class) =
                classes
                    ...
                    .position(|candidate: &ReducedFunctionalClass<'_, E>| {
                        candidate.ratio == ratio && candidate.functional == role.functional.as_ref()
                    }) {
                class
            } else {
...
fn evaluate_groups_reduced_with<F, E, const BASE_D: usize, A>(
    plan: &SetupContributionPlan<E>,
    setup_view: &RingMatrixView<'_, F, BASE_D>,
    ...
    A: ProductSum<E>,
{
    ...
    if plan.d_weights().len() != plan.d_rows()
        || scan_groups.len() != plan.groups().len()
        ...
        |acc, job| {
            ...
            for (offset, ring) in setup.iter().enumerate() {
                    ...
                    partitions.iter().zip(scan_groups).zip(&mut segment_cursors)
                {
                    ...
                    add_reduced_functional_products(base_idx, segment, direct, &mut class_scalars)?;
                }
