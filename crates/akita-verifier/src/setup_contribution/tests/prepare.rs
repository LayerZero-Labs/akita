...
fn dense_z_eq_slice_uses_relative_high_carry() {
    ...
    assert_eq!(scan.group_column_eq_slices(&plan, 0).unwrap().2, expected);
}
...
fn deferred_structured_setup_supports_empty_chunk_slots() {
    ...
    assert_eq!(
        evaluate_structured_group::<F, _>(&deferred, 0, &block_challenges, &opening_a_evals, alpha)
            .unwrap(),
        expected
    );
    assert_eq!(
        scan.evaluate_structured_group_cached::<F>(&direct, 0, &block_challenges, &opening_a_evals)
            .unwrap(),
        expected
    );
}
