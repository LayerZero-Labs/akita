...
fn structured_group_shape<'a, F, E>(
    plan: &'a SetupContributionPlan<E>,
    group_id: usize,
    block_challenges: &[E],
    opening_a_evals: &[E],
) -> Result<StructuredGroupShape<'a, E>, AkitaError>
where
...
impl<E: Field> DirectScan<E> {
    ...
    /// weights already materialized by this lifted direct scan.
    ///
...
pub(crate) fn evaluate_structured_group<F, E>(
    plan: &SetupContributionPlan<E>,
    group_id: usize,
    block_challenges: &[E],
    opening_a_evals: &[E],
    ...
    E: ExtField<F>,
{
    ...
    let z = if witness_gadget.len().is_power_of_two() {
        ...
        let weighted_base_count = checked::product([opening_a_evals.len(), a_base_offsets.len()])
            .ok_or_else(|| {
