...
impl SetupContributionFixture {
    pub fn from_shape(shape: &SetupContributionShape) -> Self {
        ...
        let total_blocks = shape.live_block_count * shape.num_claims;
        let inner_width = shape.positions_per_block * shape.depth_commit;
        ...
        let cols_per_poly_t = stride_t * shape.live_block_count;
        let n_cols_e = shape.num_claims * shape.live_block_count * shape.depth_open;
        ...
        .with_decomp(
            shape.positions_per_block,
            shape.live_block_count * shape.positions_per_block,
            shape.depth_commit,
            shape.depth_open,
        )
        ...
        let setup_contribution_inputs = SetupContributionPlanInputs {
            ...
            live_block_count: shape.live_block_count,
        ...
        let groups = vec![RelationMatrixGroupEvaluator {
            ...
            opening_a_evals: (0..shape.positions_per_block)
            ...
            n_a: shape.n_a,
            n_b: shape.n_b,
            t_cols_per_vector: shape.n_a * shape.depth_open * shape.live_block_count,
            a_row_start: 1,
        ...
        let block_index_bits = shape.live_block_count.trailing_zeros() as usize;
        let eq_low = EqPolynomial::evals(&full_vec_randomness[..block_index_bits]).unwrap();
        let z_offset_low_bits = shape.positions_per_block.trailing_zeros() as usize;
