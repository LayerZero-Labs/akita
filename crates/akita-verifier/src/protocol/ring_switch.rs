    ...
    E: FpExtEncoding<F> + FromPrimitiveInt + MulBase<F>,
{
    ...
    let group_challenges = relation.group_challenges();
    let challenges = group_challenges.first().ok_or(AkitaError::InvalidProof)?;
        ...
        let mut z_structured_contribution = E::zero();
        {
            ...
            for group in &self.groups {
                ...
                if group.block_len.is_power_of_two() {
                    ...
                    let z_hi_len = a_block_summary.len() * fold_gadget.len() * g_commit.len();
                    let eq_hi_z_table =
