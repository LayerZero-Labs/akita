...
use akita_challenges::{FoldingChallenges, TensorChallenges};
use akita_field::{
...
/// `FoldingChallenges::Flat::evals_at_pows` would produce them: one entry per
/// `(claim, block)`, packed claim-major. `Tensor` keeps the factored
/// `(left, right)` challenge set together with the cached `alpha_pows` so consumers can summarise carry
...
pub(crate) enum PreparedChallengeEvals<F: FieldCore> {
    ...
    /// `eval_factored_aggregate_at_pows`.
    Tensor {
...
impl<F: FieldCore> PreparedChallengeEvals<F> {
    ...
    /// `TensorChallenges::eval_factored_aggregate_at_pows` instead of
...
mod tests {
    ...
    fn factored_carry_summary_matches_flat_for_tensor_challenges() {
        ...
        let tensor_challenges = FoldingChallenges::Tensor(set.clone());
