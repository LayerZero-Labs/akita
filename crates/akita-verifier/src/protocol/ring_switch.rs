    ...
    E: FieldCore + MulBase<F>,
{
    ...
    Ok(PreparedMEval {
        ...
        claim_to_group,
        #[cfg(feature = "zk")]
        claim_group_sizes: group_poly_counts.to_vec(),
        num_points,
        num_eval_rows,
        gamma: gamma.to_vec(),
...
impl<E: FieldCore> PreparedMEval<E> {
        ...
        E: ExtField<F>,
    {
        ...
        #[cfg(feature = "zk")]
        let offset_blind = offset_t + t_len;
        ...
        let blind_b = if blind_digit_planes_per_group == 0 {
            ...
            let blind_segment: Vec<E> = cfg_into_iter!(0..blind_len)
                .map(|idx| {
        ...
        Ok(z_dense + w_sep + w_d + t_sep + t_b + blind_b + r_sep + r_dense)
    }
}
