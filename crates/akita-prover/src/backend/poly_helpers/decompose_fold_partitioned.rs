...
fn partition_thread_count(positions_per_block: usize) -> usize {
    #[cfg(feature = "parallel")]
    let num_threads = rayon::current_num_threads();
    #[cfg(not(feature = "parallel"))]
    let num_threads = 1;
    num_threads.min(positions_per_block.max(1)).max(1)
}
...
fn element_partitioned_decompose_fold<F: CanonicalField, const D: usize>(
    ...
    let actual_threads = partition_thread_count(positions_per_block);
...
pub fn balanced_tight_digit_fold_partitioned<const D: usize>(
    ...
    let chunks: Vec<Vec<[i32; D]>> = cfg_into_iter!(0..actual_threads)
        .map(|tid| {
            ...
            let lo = elem_start.min(positions_per_block);
            let hi = elem_end.min(positions_per_block);
            for col in lo..hi {
