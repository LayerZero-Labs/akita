...
/// `pos_in_block < positions_per_block <= u32::MAX` and `coeff_idx < D <= 65536` are pre-validated in
...
impl SingleChunkEntry {
    ...
    /// Position within the block (0..positions_per_block).
    #[inline]
...
/// Fields are private and accessed via public accessors or the
/// internal `OneHotEntry` trait. The caller-owned invariants
/// `pos_in_block < positions_per_block <= u32::MAX` and every `coeff < D <= 65536` are pre-validated in
