...
pub struct OneHotSource<'a, I: OneHotIndex> {
    ...
    /// Logical coefficients in one source chunk.
    pub chunk_size: usize,
    /// Logical multilinear variable count.
    pub num_vars: usize,
}

impl<'a, I: OneHotIndex> OneHotSource<'a, I> {
    pub(crate) fn validate_ring_dimension(self, ring_d: usize) -> Result<usize, AkitaError> {
        if ring_d == 0 || ring_d > usize::from(u16::MAX) + 1 {
            return Err(AkitaError::InvalidInput(
                "one-hot commitment ring dimension is unsupported".into(),
            ));
        }
        if !(self.chunk_size.is_multiple_of(ring_d) || ring_d.is_multiple_of(self.chunk_size)) {
            return Err(AkitaError::InvalidInput(format!(
                "onehot chunk size {} and D={ring_d} must divide one another",
                self.chunk_size
            )));
        }
    ...
    pub(crate) fn view_layout(
        ...
        if num_positions_per_block == 0
            ...
            || u32::try_from(num_positions_per_block).is_err()
        {
            return Err(AkitaError::InvalidInput(
                "one-hot positions per block are unsupported".into(),
            ));
        }
        let rings = self.validate_ring_dimension(ring_d)?;
        Ok((rings, rings.div_ceil(num_positions_per_block)))
    }
    ...
    fn hot_field_position(self, chunk_index: usize, raw: I) -> Result<usize, AkitaError> {
        chunk_index
            .checked_mul(self.chunk_size)
            .and_then(|base| base.checked_add(raw.as_usize()))
            .ok_or_else(|| AkitaError::InvalidInput("onehot source index overflow".into()))
    }

    pub(crate) fn materialize_block_range(
        self,
        ring_d: usize,
        num_positions_per_block: usize,
        block_range: std::ops::Range<usize>,
    ) -> Result<FlatBlocks<SparseRingBlockEntry>, AkitaError> {
        let (ring_count, live_blocks) = self.view_layout(ring_d, num_positions_per_block)?;
        if block_range.start > block_range.end || block_range.end > live_blocks {
            return Err(AkitaError::InvalidInput(format!(
                "one hot block range {block_range:?} exceeds {live_blocks} blocks"
            )));
        }
        ...
        let field_start = ring_start
            .checked_mul(ring_d)
            .ok_or_else(|| AkitaError::InvalidInput("one hot field range overflow".into()))?;
        let field_end = ring_end
            .checked_mul(ring_d)
            .ok_or_else(|| AkitaError::InvalidInput("one hot field range overflow".into()))?;
        let chunk_start = field_start / self.chunk_size;
        let chunk_end = field_end.div_ceil(self.chunk_size).min(self.indices.len());
        let block_count = ring_end
            .div_ceil(num_positions_per_block)
            .saturating_sub(block_range.start);
        let capacity = self.indices[chunk_start..chunk_end]
            .iter()
            .filter(|position| position.is_some())
            .count();
        let mut blocks = FlatBlocks::with_capacity(block_count, capacity);
        let mut current_block = 0;
        for (offset, position) in self.indices[chunk_start..chunk_end]
            .iter()
            .copied()
            .enumerate()
        {
            let Some(position) = position else { continue };
            let field_position = self.hot_field_position(chunk_start + offset, position)?;
            let ring_index = field_position / ring_d;
            if !(ring_start..ring_end).contains(&ring_index) {
                continue;
            }
            ...
            let coefficient = u16::try_from(field_position % ring_d).map_err(|_| {
            ...
            blocks.push_entry(
                ...
                block_index,
                block_count,
                SparseRingBlockEntry::new(position, coefficient, 1),
            )?;
        }
        blocks.finish_build(current_block, block_count)
    }
}
...
impl<F: Field, I: OneHotIndex> OneHotPoly<F, I> {
    /// Borrow the complete representation used only by commitment execution.
    pub fn commitment_source(&self) -> OneHotSource<'_, I> {
        OneHotSource {
    ...
    pub(crate) fn validate_ring_dimension(&self, ring_d: usize) -> Result<usize, AkitaError> {
        self.commitment_source().validate_ring_dimension(ring_d)
    }
    ...
    pub(super) fn view_layout(
        ...
        self.commitment_source()
            .view_layout(ring_d, num_positions_per_block)
    }
    ...
    pub(crate) fn num_live_blocks_for(
        ...
        self.commitment_source()
            .num_live_blocks_for(ring_d, num_positions_per_block)
    }
