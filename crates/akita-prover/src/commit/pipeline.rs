//! Commit pipeline: input validation, layout validation, and the shared
//! `commit_with_validated_params` execution kernel plus the explicit-params
//! entry points.
...
use akita_types::{
    AkitaCommitmentHint, AkitaExpandedSetup, LevelParams, OpeningBatch, RingCommitment,
};

use crate::commit::ajtai::backend::CommitBackend;
use crate::commit::inner::commit_inner_group;
...
pub(crate) fn checked_commit_b_input_len(
    total_polys: usize,
    per_poly: usize,
) -> Result<usize, AkitaError> {
...
pub(crate) fn commit_with_validated_params<F, const D: usize, P, B>(
    polys: &[P],
    backend: &B,
...
where
    ...
    B: CommitBackend<F>,
{
    let (decomposed_inner_rows, recomposed_inner_rows, b_input_digits) =
        commit_inner_group::<F, D, P, B>(polys, backend, prepared, params)?;
    ...
    #[cfg(feature = "zk")]
    let mut u = outer_commit::<F, D, B>(backend, prepared, params, &b_input_digits)?;
    #[cfg(not(feature = "zk"))]
    let u = outer_commit::<F, D, B>(backend, prepared, params, &b_input_digits)?;
    // ZK blinding only applies to the single-tier B image; tiered proofs are
    // exercised non-zk, matching the previous pipeline.
    #[cfg(feature = "zk")]
    if params.f_key.is_none() {
...
mod tests {
    ...
    use akita_types::{FlatDigitBlocks, SetupMatrixEnvelope, SisModulusFamily};
