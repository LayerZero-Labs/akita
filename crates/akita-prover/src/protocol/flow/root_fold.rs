...
use super::root_fold_eval::evaluate_root_claims_at_prepared_points;
pub(in crate::protocol::flow) use super::root_fold_eval::evaluate_recursive_witness_at_multiplier_point;
    ...
    match terminal_proof_mode {
        TerminalProofMode::DirectRingRelations => {
            ...
            #[cfg(not(feature = "zk"))]
            {
                ...
                if logical_w.len() != expected_w_len {
                    return Err(AkitaError::InvalidSetup(format!(
                        "scheduled root direct witness length did not match runtime witness: expected={expected_w_len}, actual={}",
                        logical_w.len()
                    )));
                }
