    ...
    ) -> Result<RecursiveSuffixOutcome<F, C>, AkitaError>,
{
    ...
    if prepared_claims
        .commitments_by_point
        .iter()
        .any(|commitment| commitment.u.len() != root_step.params.outer_commitment_rows())
    {
        return Err(AkitaError::InvalidInput(
...
// The `Intermediate` variant carries the full level-state struct;
// `Terminal` is comparatively small. Boxing isn't worth the extra
// indirection at the proving call sites, and the variant counts are
    ...
    CommitW: FnOnce(&RecursiveWitnessFlat) -> Result<NextWitnessCommitment<F>, AkitaError>,
{
    ...
    // The legacy helper iterates `u` directly after `v`, which would
    // put `u_final` into tier-1 row positions and yield a relation
    // claim inconsistent with `Σ_x M̃·w`. See `specs/tiered_commit.md`
    // §3 and the Phase 4f-sumcheck fix.
    //
    // (Both `relation_claim_from_rows_extension*` helpers now return
    // `Result` after the security-hardening pass; propagate up.)
