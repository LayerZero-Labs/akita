        ...
        FnOnce(RecursiveProverState<F, L>) -> Result<RecursiveSuffixOutcome<F, L>, AkitaError>,
{
    ...
    #[cfg(feature = "zk")]
    let zk_hiding = {
        let mut proof = zk_hiding;
        proof.hiding_witness = final_state.zk_hiding.hiding_witness.clone();
        proof
    };
    ...
    ) -> Result<RecursiveSuffixOutcome<F, C>, AkitaError>,
{
    ...
    #[cfg(feature = "zk")]
    let (zk_hiding_proof, zk_hiding_state) = build_zk_hiding_context::<F, E, C, D>(
        expanded,
        ntt_shared,
        schedule,
        &root_step.params,
