...
/// along with intermediate prover witness data (`w_hat`, `z_folded_rings`, `hint`).
///
/// M and z are never materialized on the hot path — split-eq factoring computes
/// their products on-the-fly via `compute_relation_quotient`, while debug/test code
...
#[tracing::instrument(skip_all, name = "new_ring_relation_prover")]
#[inline(never)]
pub fn new_ring_relation_prover<F, const D: usize, T, P, B>(
    backend: &B,
    prepared: &B::PreparedSetup<D>,
...
{
    {
        ...
        tracing::trace!(
            ...
            "RingRelationInstance::new_prover"
        );
    }
    ...
    for claim_idx in 0..num_claims {
        ...
        if point_idx >= incidence_summary.num_points()
            || incidence_summary.claim_poly_indices()[claim_idx] >= num_polys_per_point[point_idx]
        {
            return Err(AkitaError::InvalidInput(
    ...
    let flattened_hint = {
        ...
        for (mut hint, &group_size) in hints.into_iter().zip(num_polys_per_point.iter()) {
            if hint.decomposed_inner_rows.len() != group_size {
                return Err(AkitaError::InvalidInput(
                    "batched prover hint group sizes do not match polynomial groups".to_string(),
                ));
            }
        ...
        #[cfg(not(feature = "zk"))]
        {
            AkitaCommitmentHint::with_recomposed_inner_rows(decomposed_inner_rows, t_rows_by_poly)
        }
    };
    ...
    let d_blinding_digits = match m_row_layout {
        MRowLayout::WithDBlock => sample_blinding_digits::<F, D>(lp.d_key.row_len(), lp.log_basis)?,
    ...
    let z_folded_rings = {
        ...
        let _span = tracing::info_span!("compute_batched_z_pre", num_points = num_points).entered();
...
#[allow(clippy::too_many_arguments)]
#[tracing::instrument(skip_all, name = "new_ring_relation_recursive_multipoint_prover")]
#[inline(never)]
pub fn new_ring_relation_recursive_multipoint_prover<F, const D: usize, T, B>(
    backend: &B,
    prepared: &B::PreparedSetup<D>,
    ...
    let z_folded_rings = {
        ...
        for claim_idx in 0..num_claims {
            let challenge_offset = claim_idx.checked_mul(lp.num_blocks).ok_or_else(|| {
                AkitaError::InvalidSetup("recursive multipoint challenge offset overflow".into())
            })?;
