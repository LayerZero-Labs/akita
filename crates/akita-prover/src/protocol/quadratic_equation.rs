    ...
    B: RingSwitchComputeBackend<F>,
{
    ...
    let tiered_state = if lp.is_tiered_root() {
        ...
        // reached through the backend's prepared-setup boundary.
        ...
        for (g, &bundle_size) in num_polys_per_point.iter().enumerate() {
            ...
            for chunk_i in 0..split {
                ...
                // Route the per-chunk cyclic B' mat-vec through the
                // backend's `cyclic_digit_rows`. The backend reads each
                // B row's first `chunk.len()` cells (B'), since
                // `vec_len = chunk.len().min(inner_width)` upstream.
                let cyclic = backend.cyclic_digit_rows::<D>(prepared, n_b_prime, chunk)?;
            ...
            // F · ûhat_concat^(g) (cyclic). F is a separately-derived
            // matrix with its own local NTT cache (built above), so we
            // call the direct kernel rather than the backend.
            let f_cyclic = crate::kernels::linear::mat_vec_mul_ntt_single_i8_cyclic::<F, D>(
                &f_ntt,
                n_f,
                f_width,
                uhat_planes,
