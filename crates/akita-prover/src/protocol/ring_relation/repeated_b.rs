...
mod tests {
    ...
    fn nonuniform_groups_use_max_group_row_width() {
        ...
        #[cfg(feature = "zk")]
        let b_blinding = vec![FlatDigitBlocks::<D>::empty(); num_polys_per_segment.len()];
    ...
    fn nonuniform_groups_insert_group_local_blinding() {
        ...
        let log_basis = 3;
        let row_width = num_polys_per_segment
            .iter()
        ...
        let blocks: Vec<Vec<[i8; D]>> = (0..3)
            ...
            .collect();
        let t_hat = FlatDigitBlocks::from_blocks(blocks);
        let b_blinding: Vec<FlatDigitBlocks<D>> = (0..num_polys_per_segment.len())
            .map(|group| {
                FlatDigitBlocks::new(
                    (0..blinding_width)
                        .map(|plane| {
