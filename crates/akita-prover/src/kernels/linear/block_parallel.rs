...
/// parallelism (few tiles) while this path gives live_block_count-way parallelism.
...
>(
    ...
    cfg_into_iter!(0..live_block_count)
        .map(|block_idx| {
            ...
            for col in 0..positions_per_block {
                let seq = block_idx + col * live_block_count;
...
>(
    ...
    if inner_width == 0 {
        return vec![CyclotomicRing::<F, D>::zero(); live_block_count];
    }
    ...
    if live_block_count < SMALL_ROW_BLOCK_PARALLEL_MIN_BLOCKS {
