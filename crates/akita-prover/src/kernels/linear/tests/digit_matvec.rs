...
fn mat_vec_mul_digits_i8_strided_matches_block_path_across_batch_boundary() {
    ...
    let mut strided_digits = Vec::with_capacity(live_block_count * digits_per_block);
...
fn mat_vec_mul_digits_i8_sparse_batches_match_schoolbook() {
    ...
    const D: usize = 64;
    let log_basis = 3;
    let live_block_count = 7;
    let digits_per_block = 5;
    let total_planes = live_block_count * digits_per_block;
    ...
    let positions: Vec<(usize, usize)> = (0..total_planes)
        .map(|idx| {
            ...
            (permuted % live_block_count, permuted / live_block_count)
        })
        .collect();

    for live_count in [0usize, 15, 16, 17, 31] {
        let mut digit_blocks = vec![vec![[0i8; D]; digits_per_block]; live_block_count];
...
fn mat_vec_mul_i8_strided_matches_direct_digits_on_multi_tile_path() {
    ...
    let mut strided_coeffs = Vec::with_capacity(live_block_count * rings_per_block);
