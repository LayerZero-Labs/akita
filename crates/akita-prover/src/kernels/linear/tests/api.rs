...
fn raw_i8_strided_accepts_signed_unit_outside_binary_digit_range() {
    ...
    let slot = build_ntt_slot(
        flat.ring_view::<D>(num_rows, fold_position_count)
            .expect("valid matrix view"),
    )
    .expect("Q32 dispatch should support this field and ring dimension");
    let coeffs: Vec<[i8; D]> = (0..fold_position_count)
        .flat_map(|col| {
            (0..live_fold_count).map(move |block| {
                if (col + block) % 2 == 0 {
                    [1i8; D]
                } else {
                    [-1i8; D]
        ...
        .collect();
    let blocks: Vec<Vec<[i8; D]>> = (0..live_fold_count)
        .map(|block| {
            (0..fold_position_count)
                .map(|col| coeffs[col * live_fold_count + block])
                .collect()
        })
        .collect();
