...
/// `live_block_count` groups of `n_a` ring rows.
///
    ...
    Acc: Fn(&mut [Vec<CyclotomicCrtNtt<W, K, D>>], usize, usize) + Sync,
{
    if inner_width <= safe_width {
        ...
        let final_accs: Vec<Vec<CyclotomicCrtNtt<W, K, D>>> = cfg_fold_reduce!(
            0..num_tiles,
            || vec![vec![CyclotomicCrtNtt::<W, K, D>::zero(); n_a]; live_block_count],
    ...
    cfg_fold_reduce!(
        0..num_chunks,
        || vec![vec![CyclotomicRing::<F, D>::zero(); n_a]; live_block_count],
        |mut out: Vec<Vec<CyclotomicRing<F, D>>>, chunk_idx| {
            ...
            let mut accs = vec![vec![CyclotomicCrtNtt::<W, K, D>::zero(); n_a]; live_block_count];
            accumulate(&mut accs, tile_start, tile_end);
