...
/// `num_live_blocks` groups of `n_a` ring rows.
///
    ...
    Acc: Fn(&mut [Vec<CyclotomicCrtNtt<W, K, D>>], usize, usize) + Sync,
{
    if inner_width <= safe_width {
        ...
        let final_accs: Vec<Vec<CyclotomicCrtNtt<W, K, D>>> = cfg_fold_reduce!(
            0..num_tiles,
            || vec![vec![CyclotomicCrtNtt::<W, K, D>::zero(); n_a]; num_live_blocks],
    ...
    cfg_fold_reduce!(
        0..num_chunks,
        || vec![vec![CyclotomicRing::<F, D>::zero(); n_a]; num_live_blocks],
        |mut out: Vec<Vec<CyclotomicRing<F, D>>>, chunk_idx| {
            ...
            let mut accs = vec![vec![CyclotomicCrtNtt::<W, K, D>::zero(); n_a]; num_live_blocks];
            accumulate(&mut accs, tile_start, tile_end);
