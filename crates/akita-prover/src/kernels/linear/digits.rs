...
>(
    ...
    if inner_width == 0 || n_a == 0 {
        return vec![vec![CyclotomicRing::<F, D>::zero(); n_a]; live_fold_count];
    }
    ...
    if n_a <= SMALL_ROW_BLOCK_PARALLEL_MAX_ROWS
        && live_fold_count >= SMALL_ROW_BLOCK_PARALLEL_MIN_BLOCKS
...
>(
    ...
    let n_a = ntt_mat.len();
    let mat_width = ntt_mat.first().map_or(0, |row| row.len());
    let inner_width = mat_width.min(fold_position_count);
    if inner_width == 0 || n_a == 0 {
        return vec![vec![CyclotomicRing::<F, D>::zero(); n_a]; live_fold_count];
    }
    ...
    debug_assert!(
        digit_rows_within_digit_bound::<D>(
            coeffs,
            inner_width.saturating_mul(live_fold_count),
            digit_bound
        ),
    ...
    drive_block_chunked_matvec(
        ...
        |accs, start, end| {
            for col in start..end {
                let seq_start = col * live_fold_count;
                if seq_start >= coeffs.len() {
                    break;
                }
                let live_blocks = live_fold_count.min(coeffs.len() - seq_start);
...
>(
    ...
    let rhs_bound = strided_i8_abs_bound(coeffs, live_fold_count, inner_width);
    ...
    if n_a <= SMALL_ROW_BLOCK_PARALLEL_MAX_ROWS && inner_width <= safe_width {
        ...
        if live_fold_count >= SMALL_ROW_BLOCK_PARALLEL_MIN_BLOCKS || live_fold_count >= num_tiles {
...
fn strided_i8_abs_bound<const D: usize>(
    ...
    for col in 0..inner_width {
        let Some(seq_start) = col.checked_mul(live_fold_count) else {
            break;
        };
        if seq_start >= coeffs.len() {
            break;
