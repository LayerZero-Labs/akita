...
pub(super) const I8_RHS_MAX_ABS: u32 = 31;
...
>(
    ...
    let rhs_bits = bit_len_u128(rhs_max_abs.max(1) as u128);
    let terms_bits = bit_len_usize(D.saturating_sub(1).max(1));
...
pub(super) fn max_centered_abs_u32<F: CanonicalField, const D: usize>(
    ...
    for ring in rings {
        for coeff in ring.coefficients() {
            ...
            let abs_u32 = u32::try_from(abs).ok()?;
            max_abs = max_abs.max(abs_u32);
        }
    }
    Some(max_abs)
}
