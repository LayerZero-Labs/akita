    ...
    F: FieldCore + CanonicalField,
{
    ...
    if inner.decomposed_inner_rows.block_count() != live_block_count {
        return Err(AkitaError::InvalidSetup(format!(
...
where
    F: FieldCore + CanonicalField,
{
    if params.live_block_count == 0 || params.positions_per_block == 0 {
        return Err(AkitaError::InvalidSetup(
            "commit params require nonzero live_block_count and positions_per_block".to_string(),
        ));
    }
    ...
    if params.a_key.col_len() != expected_a_width {
        return Err(AkitaError::InvalidSetup(format!(
            "commit params A width {} does not match positions_per_block * num_digits_commit = {expected_a_width}",
...
mod tests {
    ...
    fn commit_inner_shape_accepts_many_all_zero_blocks() {
        let live_block_count = 1024;
        let inner = inner_witness(live_block_count, 3, vec![6; live_block_count]);
        validate_commit_inner_shape::<F, D>(&inner, live_block_count, 3, 2, 4)
