...
use super::{DigitRowsComputeBackend, OperationCtx};
    ...
    B: DigitRowsComputeBackend<F>,
{
    ...
    let outputs = ctx.backend().compression_rows(ctx.prepared(), &views)?;
    ...
    for (((&item_index, rows), packed_digits), _) in
        item_indices.iter().zip(outputs).zip(packed).zip(expanded)
    {
        if rows.len() != first_map.output_rank() {
            return Err(AkitaError::InvalidSetup(
...
mod tests {
    ...
    fn prepared_context(
        ...
        let setup = AkitaProverSetup::<F>::generate_with_capacity(
            ...
            128,
            SetupMatrixEnvelope {
                max_setup_len: max_flat_coefficients,
            },
        )
        .unwrap();
        let prepared = CpuBackend
            .prepare_expanded::<128>(setup.expanded.clone())
            .unwrap();
        (setup, prepared)
    }
