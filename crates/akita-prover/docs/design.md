...
> **Superseded state model.** The arithmetic and source-routing background in
> this document remains useful, but its registry, registrar, composite
> dispatcher, and mandatory split-builder design have been replaced by the
> [simplified commitment backend composition](../../../specs/commitment-execution-simplification.md).
> Use that specification and the current source for backend implementation.
...
When both paths exist, request compilation selects the external path only when
the complete source group advertises one homogeneous compatible capability.
Otherwise it selects the standard polynomial path for the complete group. Tests
compare external execution with a separately constructed standard reference.
...
standard and external paths is tested for equality against a separately
constructed standard reference. Route selection is deterministic: one
homogeneous external capability is preferred for the complete source group;
otherwise every source must support a standard representation and the complete
group uses that route. A request that supports neither homogeneous choice is
rejected during request compilation.
...
pub trait InnerCommitOperation<F: Field>: Send + Sync {
    fn commit_inner(
        &self,
        registrar: &StateRegistrar<'_, InnerImage>,
        plan: &CommitInnerPlan,
...
pub trait CompressionOperation<F: Field>: Send + Sync {
    fn compress(
        &self,
        registrar: &StateRegistrar<'_, CompressionState>,
...
`StateRegistrar<K>` is issued by the executor for exactly one state-producing
call. It is the checked, owner-bound public factory that lets an external
operation register backend-owned state and receive a `BackendStateRef<K>`.
Registration supplies immutable shape/binding metadata, retained-byte
accounting, and cleanup policy. External code never chooses store IDs, owner
IDs, slots, generations, or semantic marker kinds directly. The outer stage
does not receive a registrar because it produces no retained state; its complete
inner-plus-outer plan carries the metadata needed to consume its input.
...
`StateRegistrar::reserve` returns a pending deposit for that registrar's
already-validated owner containing a new `OwnerStateToken`. The operation uses that
...
Each typed executor registration keeps one stage's operation together with the
metadata that belongs to it. The inner registration owns its state owner,
source capabilities, dimensions, and optional exporter; outer owns its input
owner and dimensions; compression owns its state owner, complete compression
capabilities, and optional exporter; fused owns the corresponding fused
metadata. Common stage metadata contains only operation identity, backend
instance, diagnostic name, and resource control. In particular, compression
dimensions have one source of truth in `CompressionOperationCapabilities`, and
the stage slot determines NTT routing without a second stored routing tag.
...
- optional exporters attached to the operations whose state they understand;
- one composite-state dispatcher shared by split and fused routes; and
...
3. prefers one homogeneous external capability for the complete group, then
  falls back to operation-preferred standard representations when every source
  supports one;
...
executor.execute_full(plan, resolved_sources):
  if the executor explicitly selects fusion:
      ...
      u = outer.commit_outer(uncompressed_plan, inner_input)
      inner_outer = { inner, u }
...
The standard `ResidentCommitmentState<F>` uses a private representation, so
external operations never match on it. One built-in
`CommitmentStateDispatcher` owns the field-independent composite-state table for every split
and fused route. Each checked component carrier also contains the exact inner
and compression exporters selected for that execution. A resident policy moves
those exporter capabilities into the returned resident state alongside its
composite handle; the dispatcher table itself stores only backend-neutral
component handles. Consequently split and fused states share one composite
owner and table without ever applying a split exporter to fused state or vice
versa. A third-party GPU neither inspects a private enum nor downcasts another
owner's payload.
...
policy consumes `CommitmentStateComponents<F>`, invokes those owner-dispatched
...
4. Add the independent state-consumer, portable export/import, and
  recomputation contracts, plus the executor-issued
   typed `StateRegistrar<K>` required by state-producing external operations.
...
- [ ] A group that cannot select either one homogeneous external capability or
      standard representations for every source fails during request compilation.
...
plan types, `StateRegistrar`, `BackendStateRef`,
...
pub trait CommitmentStatePolicy<F: Field> {
    ...
    fn bind(
        ...
        dispatcher: &CommitmentStateDispatcher,
    ) -> Result<Self::State, AkitaError>;
}
```

`CommitmentStateComponents<F>` is a checked consuming carrier for the binding,
inner reference, optional compression reference, and the route-specific
exporters that understand those references. A resident policy stores the
backend-neutral handles in the one composite dispatcher and keeps the selected
exporters with `ResidentCommitmentState<F>`; a portable policy consumes the
components immediately and builds one hint. No policy gets raw store access.
