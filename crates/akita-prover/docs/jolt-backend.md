...
| Author         | Omid Bodaghi                                                                                                                    |
| Revised        | 2026-09-08                                                                                                                      |
| Status         | Implemented on the Stage 7 integration branches                                                                                 |
| Akita design   | `design.md`, replayed onto Akita `69438de6cd8ce8ed7c9ebb21bdf60b813e8fcabc`                                                       |
| Jolt evidence  | `a16z/jolt` `main` at `fc4d481ff547d65de4bb51ca54391534492d5ca8`                                                               |
| Jolt Akita pin | `69438de6cd8ce8ed7c9ebb21bdf60b813e8fcabc`                                                                                      |
...
changing its Akita protocol. Each commitment constructs a checked
`CommitmentExecutor<F, ResidentStatePolicy>` bound to the selected expanded
setup and its prepared CPU resources. A per-call `CommitmentExecutionPlan`
...
`ResidentCommitmentState<AkitaField>` rather than requiring a portable
...
The normal Jolt route selects `ResidentCommitmentState<AkitaField>` as the state
...
pub struct AkitaProverHint {
    ...
    state: Option<ResidentCommitmentState<AkitaField>>,
...
4. Ask the selected `ResidentCommitmentState` consumer for the outer-compression,
...
full committed group and `ResidentCommitmentState` contract described above.
...
- Construct dense and one-hot executors from their respective setup/prepared
pairs, keeping each executor bound to exactly one expanded setup.
...
appropriate setup-bound `CommitmentExecutor`.
...
`ResidentCommitmentState<AkitaField>`.
...
1. Extend `AkitaProverHint` and Jolt's internal opening-data plumbing to carry
  `ResidentCommitmentState` and the full committed group.
...
- Jolt's normal proving path retains `ResidentCommitmentState`, with portable export
