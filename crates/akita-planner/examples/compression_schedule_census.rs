...
use akita_types::{
    AkitaScheduleLookupKey, CompressionChainPlan, FoldSchedule, OpenCommitMatrixParams,
...
fn record_schedule(
    ...
    for (index, group) in root.precommitted_groups.iter().enumerate() {
        record_outer(
            ...
            profile,
            field_bytes,
            &group.commitment.outer_commit_matrix,
        )?;
    }
    record_outer(
        stats,
...
fn main() -> Result<(), String> {
    ...
    for family in ALL_GENERATED_FAMILIES {
            ...
            .map_err(|error| format!("{} group keys: {error}", family.module_name))?
        {
            ...
            let schedule = match (family.regen_group_batch)(AkitaScheduleLookupKey {
