...
pub(crate) trait PackingWeightAccessor<E: Field>: Sync {
...
impl<E: Field> PackingWeightAccessor<E>
    for crate::arithmetic::coefficient_packing::FusedPackingWeights<'_, E>
{
    fn point(&self) -> &akita_types::PreparedSubringCoefficientPackingPoint<E> {
        self.point()
    }
...
pub(crate) fn onehot_coefficient_packing_partials<F, E, const D: usize, I, W>(
    source: OneHotBatchView<'_, F, D, I>,
    weights: &W,
    ...
    I: OneHotIndex,
{
    fn coefficient_packing_partials_batch(
        ...
        onehot_coefficient_packing_batch(source, plan)
    }
}
    ...
    F: Field + CanonicalEncoding + Unreduced,
{
    ...
    pub(crate) fn evaluate_and_fold_subfield<const D: usize>(
        ...
        crate::sources::poly_helpers::fused_evaluate_and_fold_subfield(
...
/// Canonical sparse packing policy, shared with the named arithmetic benchmark.
pub(crate) fn onehot_coefficient_packing_batch<F, E, const D: usize, I>(
    source: OneHotBatchView<'_, F, D, I>,
    plan: SubringCoefficientPackingPlan<'_, E>,
) -> Result<Vec<SubringCoefficientPackingPartials<F>>, AkitaError>
where
...
{
    let fused_len =
        crate::arithmetic::coefficient_packing::FusedPackingWeights::<E>::required_len(plan.point)?;
    let should_prepare =
        akita_error::checked::sum(source.polys.iter().map(|poly| poly.indices.len()))
            .is_some_and(|max_hot| max_hot >= fused_len);
    if should_prepare {
        let weights = crate::arithmetic::coefficient_packing::FusedPackingWeights::new(plan.point)?;
