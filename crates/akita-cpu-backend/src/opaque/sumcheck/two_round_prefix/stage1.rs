...
pub(super) fn build_stage1_prefix_grid<E: Field + Ring + Unreduced>(
    quad_class_weights: &[E],
    b: usize,
) -> Stage1PrefixGrid<E> {
...
impl<E: Field + Ring> Stage1PrefixCache<E> {
    ...
    #[cfg(test)]
    pub(super) fn b4_test_data(&self) -> Option<([[E; 3]; 3], E, E)> {
        match self {
    ...
    #[cfg(test)]
    pub(super) fn b8_test_data(&self) -> Option<([E; 25], E, E)> {
        match self {
            Self::B4(_) => None,
