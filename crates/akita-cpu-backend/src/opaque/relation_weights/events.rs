...
pub(crate) struct RelationWeightEvents<E: Field> {
...
impl<E: Field> RelationWeightEvents<E> {
    ...
    #[must_use]
    #[cfg(test)]
    pub(crate) fn events(&self) -> &[RelationWeightEvent<E>] {
        &self.events
    }
    ...
    pub(crate) fn factor_common_alpha(&self) -> Result<RelationWeightFactorization<E>, AkitaError> {
        if self.setup_is_deferred {
            return Err(AkitaError::InvalidSetup(
    ...
    #[cfg(test)]
    pub(crate) fn evaluate_at_point(
        &self,
        point: &[E],
