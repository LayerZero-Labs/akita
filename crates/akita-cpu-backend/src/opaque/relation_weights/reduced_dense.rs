...
pub(crate) fn build_reduced_dense_relation_weights<F, E>(
...
) -> Result<crate::opaque::RelationWeightDescription<E>, AkitaError>
where
    F: Field + CanonicalEncoding,
    E: FpExtEncoding<F> + ExtField<F> + MulBaseUnreduced<F>,
{
    ...
    Ok(
        crate::opaque::RelationWeightDescription::ReducedEvaluations {
            evaluations: dense,
            live_len: compilation.witness_layout.live_coeff_len(),
        },
    )
}
