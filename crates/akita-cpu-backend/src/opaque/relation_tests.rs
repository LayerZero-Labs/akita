use crate::opaque::compression::{execute_compression_chains, CompressionExecutionInput};

use crate::opaque::{
    balanced_decompose_centered_i32_i8_into, cpu_recursive_witness_build,
    quotient_decomposition_calls, reset_quotient_decomposition_calls,
};
...
fn reduced_group_witness(
    ...
    RingRelationGroupWitness::from_parts(
        ...
        crate::opaque::OpaqueInnerRelationState::new(
            crate::commitment::InnerRelationStateMaterial::new(
                &crate::opaque::CommitInnerPlan::from_profile(&group_params.profile),
                1,
...
fn assert_reduced_compiler_matches_structured_verifier(
    ...
    let dense = crate::opaque::relation_weights::build_reduced_dense_relation_weights(
        setup,
        instance,
        fixture.alpha,
        params,
    ...
    let crate::opaque::RelationWeightDescription::ReducedEvaluations {
        evaluations: dense, ..
    } = dense
    else {
        panic!("expected reduced evaluations")
    };
    let dense_evaluation = multilinear_eval(&dense, &fixture.point).expect("dense MLE");
    assert_eq!(
        dense_evaluation,
        structured_reduced_evaluation(params, instance, setup, &fixture),
    );
    assert!(&dense[fixture.live_len..].iter().all(ReducedF::is_zero));
}
...
fn assert_reduced_compression_report(
    report: &crate::opaque::compression::CompressionExecutionReport,
) {
...
fn build_reduced_without_quotients(
    ...
    let flat = cpu_recursive_witness_build(
        instance,
        witness,
        ctx,
        ctx,
    ...
    layout
        ...
        .flat_map(|layer| {
            let outer = compression
                .source_for_test(CompressionSourceId::Outer { group_index: 0 })
                .expect("outer source");
            ...
            [
                (
                    f_span.clone(),
                    expected_packed_digits(
                        &f_span,
                        &outer.witness_for_test().stages()[layer.map_index()],
                    ),
                ),
                (
                    h_span.clone(),
                    expected_packed_digits(
                        &h_span,
                        &opening.witness_for_test().stages()[layer.map_index()],
                    ),
                ),
            ]
        })
...
fn reduced_ring_switch_rejects_quotient_witness_without_running_quotients() {
    ...
    with_reduced_setup(setup_coefficients, |ctx, _| {
        ...
        reset_multi_group_quotient_calls();
        reset_quotient_decomposition_calls();
        assert!(cpu_recursive_witness_build(
            &instance,
            witness,
            ctx,
            ctx,
