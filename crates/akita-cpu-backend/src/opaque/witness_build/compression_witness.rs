//! Consumer-owned materialization of B/D compression chains.
...
pub(crate) struct CompressionSourceWitness<F: Field> {
    pub(super) id: CompressionSourceId,
    material: PortableCompressionState<F>,
    pub(super) terminal: CompressionTerminalPayload<F>,
}
...
impl<F: Field> CompressionSourceWitness<F> {
    pub(super) fn witness(&self) -> &CompressionChainWitness {
    ...
    pub(super) fn quotient(&self, map_index: usize) -> Result<&RingVec<F>, AkitaError> {
        self.material
            .quotients()
