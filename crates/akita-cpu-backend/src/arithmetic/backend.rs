...
pub(crate) struct CompressionRowsProducts<F: Field, const D: usize> {
...
pub(crate) trait CompressionComputeBackend<F>: ComputeBackendSetup<F>
where
    F: Field + CanonicalEncoding,
{
...
pub(crate) trait DigitRowsComputeBackend<F>:
...
#[cfg(test)]
pub(crate) trait CyclicRowsComputeBackend<F>: DigitRowsComputeBackend<F>
where
    F: Field + CanonicalEncoding,
{
