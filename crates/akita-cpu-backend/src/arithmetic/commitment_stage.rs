...
impl<F: Field + 'static> CpuInnerImageStore<F> {
    fn with_witnesses<R>(
        ...
        consume: impl FnOnce(&[RingVec<F>]) -> Result<R, AkitaError>,
    ) -> Result<R, AkitaError> {
        consume(self.owner.value::<Vec<RingVec<F>>>(image)?)
    }
...
pub(crate) struct CpuInnerCommitOperation<'a, F: Field> {
...
impl<'a, F: Field> CpuInnerCommitOperation<'a, F> {
    ...
    pub(crate) fn new(backend: &'a CpuBackend, prepared: &'a CpuPreparedSetup<F>) -> Self {
        Self {
            backend,
            prepared,
    ...
    pub(crate) fn owner(&self) -> &StateOwnerCapability<InnerImage> {
        &self.storage.owner
    }

    /// Exporter for crossing from this resident CPU image to another stage owner.
    pub(crate) fn portable_exporter(&self) -> Arc<dyn InnerImageExportOperation<F>>
    where
        F: 'static,
    {
    ...
    F: Field + CanonicalEncoding + Unreduced + WithCommitAccumulator + 'static,
{
    fn commit_inner(
        ...
        let retained_coefficients = images.iter().try_fold(0usize, |total, image| {
            total.checked_add(image.coeff_len()).ok_or_else(|| {
...
pub(crate) struct CpuOuterCommitOperation<'a, F: Field> {
    backend: &'a CpuBackend,
    prepared: &'a CpuPreparedSetup<F>,
        ...
        match inner {
            InnerImageInput::Owned(image) => {
                ...
                self.inner_storage.with_witnesses(image, |witnesses| {
                    let rows = witnesses.iter().collect::<Vec<_>>();
...
mod tests {
    ...
    fn object_safe_cpu_inner_stage_retains_and_exports_exact_rows() {
        ...
        let resident_pointer = operation
            .owner()
            .value::<Vec<RingVec<F>>>(output.image())
            .unwrap()[0]
            .coeffs()
            .as_ptr();
        let moved = operation
