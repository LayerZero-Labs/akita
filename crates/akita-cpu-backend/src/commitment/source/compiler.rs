...
pub(crate) struct CompiledCommitmentRequest<'a, F: Field> {
...
impl<'a, F: Field> CompiledCommitmentRequest<'a, F> {
    ...
    pub(crate) fn selected_polynomial_types(&self) -> impl Iterator<Item = PolynomialType> + '_ {
    ...
    pub(crate) fn materialize(self) -> Result<Vec<ResolvedCommitSource<'a, F>>, AkitaError> {
        self.sources
            .iter()
...
pub(crate) fn compile_commitment_request<'a, F: Field>(
    plan: &CommitInnerPlan,
    sources: &'a [&'a dyn CommitmentSource<F>],
