...
/// Opaque reusable commitment state transported from commitment to proving.
///
/// CPU storage and persistence codecs belong to the backend. Only public shape
/// metadata can be inspected by protocol orchestration or application code.
///
/// ```compile_fail,E0624
/// use akita_prover::PortableCommitmentHandle;
/// use jolt_field::Prime128Offset275 as F;
/// fn inspect(handle: &PortableCommitmentHandle<F>) { let _ = handle.inner_rows(); }
/// ```
/// ```compile_fail,E0599
/// use akita_prover::PortableCommitmentHandle;
/// use akita_serialization::AkitaSerialize;
/// use jolt_field::Prime128Offset275 as F;
/// fn export(handle: &PortableCommitmentHandle<F>) {
///     handle.serialize_uncompressed(Vec::new()).unwrap();
/// }
/// ```
#[derive(Clone, PartialEq, Eq)]
pub(crate) struct PortableCommitmentHandle<F: Field> {
...
impl<F: Field> core::fmt::Debug for PortableCommitmentHandle<F> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("PortableCommitmentHandle")
            .field("ring_dimension", &self.ring_dim)
            .field("source_count", &self.inner_rows.len())
            .finish_non_exhaustive()
    }
}

impl<F: Field> PortableCommitmentHandle<F> {
    ...
    pub(crate) fn new(ring_dim: usize, inner_rows: Vec<RingVec<F>>) -> Result<Self, AkitaError> {
        let hint = Self {
            inner_rows,
            ring_dim,
    ...
    #[cfg(test)]
    pub(crate) fn singleton_with_outer_compression(
    ...
    pub(crate) fn singleton_with_reduced_outer_compression(
        inner_rows: RingVec<F>,
        witness: &CompressionChainWitness,
    ) -> Result<Self, AkitaError> {
        let hint = Self {
    ...
    pub(crate) fn reduced_outer_compression_witness(
        &self,
        plan: &CompressionChainPlan,
    ) -> Result<CompressionChainWitness, AkitaError> {
    ...
    pub(crate) fn outer_compression_quotients(
        &self,
        plan: &CompressionChainPlan,
    ...
    pub(crate) fn validate_outer_compression(
        &self,
        plan: &CompressionChainPlan,
    ) -> Result<(), AkitaError> {
    ...
    pub(crate) fn validate_outer_compression_mode(
        &self,
        plan: &CompressionChainPlan,
...
impl<F: Field + Valid> Valid for PortableCommitmentHandle<F> {
...
impl<F: Field + AkitaSerialize> PortableCommitmentHandle<F> {
    pub(crate) fn serialize_with_mode<W: Write>(
        &self,
        mut writer: W,
        compress: Compress,
    ) -> Result<(), SerializationError> {
    ...
    pub(crate) fn serialized_size(&self, compress: Compress) -> usize {
...
impl<F> PortableCommitmentHandle<F>
where
    F: Field + Valid + AkitaDeserialize<Context = ()>,
{
    pub(crate) fn deserialize_with_mode<R: Read>(
        mut reader: R,
        compress: Compress,
        validate: Validate,
        _ctx: &(),
...
#[cfg(test)]
impl<F: Field + Valid + AkitaDeserialize<Context = ()>> PortableCommitmentHandle<F> {
    fn deserialize_uncompressed<R: Read>(reader: R, ctx: &()) -> Result<Self, SerializationError> {
...
mod tests {
    ...
    fn hint_encoding_is_polynomial_count_shared_dimension_then_field_rows() {
        let hint = PortableCommitmentHandle::new(4, vec![rows(10, 8, 4), rows(30, 8, 4)]).unwrap();
        ...
        let decoded =
            PortableCommitmentHandle::<F>::deserialize_uncompressed(&encoded[..], &()).unwrap();
    ...
    fn hint_constructor_rejects_inconsistent_semantic_rows() {
        assert!(PortableCommitmentHandle::<F>::new(0, Vec::new()).is_err());
        assert!(PortableCommitmentHandle::new(4, vec![rows(1, 8, 4), rows(2, 12, 4)]).is_err());
        assert!(PortableCommitmentHandle::new(4, vec![rows(1, 8, 4), rows(2, 8, 2)]).is_err());
    }
    ...
    fn hint_decoder_rejects_nonintegral_and_oversized_shapes() {
        ...
        assert!(
            PortableCommitmentHandle::<F>::deserialize_uncompressed(&nonintegral[..], &()).is_err()
        );
        ...
        assert!(
            PortableCommitmentHandle::<F>::deserialize_uncompressed(&oversized[..], &()).is_err()
        );
    }
    ...
    fn hint_round_trips_exactly_two_derived_compression_stages() {
        ...
        let quotients = plan
            ...
            .collect::<Vec<_>>();
        let hint = PortableCommitmentHandle::new_with_outer_compression(
            4,
            vec![rows(10, 8, 4)],
            &witness,
            &quotients,
    ...
    fn reduced_hint_round_trips_compression_stages_without_quotients() {
        ...
        let stages = plan
            ...
            .unwrap();
        let witness = CompressionChainWitness::new(plan.clone(), stages).unwrap();
        let hint = PortableCommitmentHandle::singleton_with_reduced_outer_compression(
            rows(10, 8, 4),
            &witness,
        )
        .unwrap();
