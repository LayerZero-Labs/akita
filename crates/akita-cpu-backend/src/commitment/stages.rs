...
pub(crate) struct UncompressedCommitmentOutput<F: Field> {
    image: BackendStateRef<InnerImage>,
    u: RingVec<F>,
}
...
impl<F: Field> UncompressedCommitmentOutput<F> {
    ...
    #[cfg(test)]
    pub(crate) const fn u(&self) -> &RingVec<F> {
        &self.u
    }

    /// Consume into the resident image and canonical B image.
    pub(crate) fn into_parts(self) -> (BackendStateRef<InnerImage>, RingVec<F>) {
        (self.image, self.u)
    }
}

/// Output of commitment compression with mode-bound retained state.
pub(crate) struct CompressionStageOutput<F: Field> {
...
pub(crate) struct FullCommitmentOutput<F: Field> {
    image: BackendStateRef<InnerImage>,
...
impl<F: Field> FullCommitmentOutput<F> {
    ...
    pub(crate) fn into_parts(self) -> (BackendStateRef<InnerImage>, CompressionStageOutput<F>) {
...
impl<F: Field> CompressionStageOutput<F> {
    ...
    pub(crate) const fn state(&self) -> &BackendStateRef<CompressionState> {
        &self.state
    }

    /// Consume into the public terminal payload and retained state.
    pub(crate) fn into_parts(self) -> (RingVec<F>, BackendStateRef<CompressionState>) {
...
pub(crate) trait CompressionOperation<F: Field>: Send + Sync {
...
pub(crate) trait FusedInnerOuterOperation<F: Field>: Send + Sync {
    ...
    fn compression_state(relation_mode: RingRelationMode) -> BackendStateRef<CompressionState> {
        ...
        let binding = CommitmentStateBinding::new(
            AkitaSetupDescriptor {
                ...
                setup_seed: AkitaSetupSeed::shake256_paged_v1([9; 32]),
            },
            CommitInnerPlan {
                ring_dimension: 64,
                num_live_blocks: 1,
