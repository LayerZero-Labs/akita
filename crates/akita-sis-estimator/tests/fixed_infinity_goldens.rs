...
#[test]
fn fixed_infinity_zgsa_goldens_match_supported_rows_and_reject_invalid_volume_rows() {
    run_fixed_infinity_goldens(
...
fn run_fixed_infinity_goldens(
    ...
    for row in parse_rows(csv) {
        ...
        let result = cost_infinity(row.beta_input, &params, row.zeta_input, &config);
