...
/// Legacy production rows regenerated by the documented full-table command.
///
/// Bound one is a compression-only authority extension and is emitted below
/// only for the audited dispatch pairs in `COMPRESSION_BOUND_ONE_DIMS`.
const GENERATED_COEFF_LINF_BUCKETS: &[u64] = &[
...
/// Exact-bound-one rows required by compression dispatch and fixtures.
pub const COMPRESSION_BOUND_ONE_DIMS: &[(AkitaModulusFamily, u32)] = &[
    (AkitaModulusFamily::Q128, 8),
    (AkitaModulusFamily::Q128, 16),
    (AkitaModulusFamily::Q128, 32),
    (AkitaModulusFamily::Q128, 64),
...
impl Default for InfinityWidthTableConfig {
    fn default() -> Self {
        Self {
            ...
            coeff_linf_bounds: GENERATED_COEFF_LINF_BUCKETS.to_vec(),
...
pub fn is_full_infinity_width_table_config(config: &InfinityWidthTableConfig) -> bool {
    same_set(&config.families, FAMILIES)
        ...
        && same_set(&config.coeff_linf_bounds, GENERATED_COEFF_LINF_BUCKETS)
...
pub fn generate_infinity_width_rows(
    ...
    if is_full_infinity_width_table_config(config) {
        for &(family, d) in COMPRESSION_BOUND_ONE_DIMS {
            for rank in 1..=config.max_rank {
